...
Eight MCP servers are menu rows, all off by default:
...
- **Perplexity** (row 25) — `claude mcp add perplexity --env PERPLEXITY_API_KEY=<key> -- npx -y @perplexity-ai/mcp-server`. The web-grounded search, ask, research and reason tools the [`web-research`](skills/web-research/) skill calls. The key is per-account and metered, so it is not baked in: pass `--perplexity-api-key` / `-PerplexityApiKey`, or export `PERPLEXITY_API_KEY` before the run. With neither — or with a blank value — the item prints where to create one and **skips rather than registering a server that can never authenticate**. The item's own output names the flag and the environment variable, never the value, and an unknown option is reported as `--some-flag=<redacted>` — until 2026-09-14 it was echoed whole, so `--perplexity-api-key=<key>` (the `=` spelling, which the parser did not accept) printed the key and then ran on as though none had been given. Both key flags now take that spelling. What `claude mcp add` itself writes to the terminal is the CLI's business, not the script's.
