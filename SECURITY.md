...
- Know that it will install and run `npx`-fetched packages (`skills`, `@opengsd/gsd-core`, `claude-mem`) and add third-party Claude Code plugin marketplaces (Superpowers, `anthropics/claude-code`, `excalidraw-generator`) — see [`MARKETPLACE.md`](MARKETPLACE.md) section 3 for exactly what each one is and does.
