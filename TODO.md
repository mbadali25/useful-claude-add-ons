...
- (f) `docs/diagrams/process-crew-brief.mmd` is anchored at 60c79407 and still draws the PM brief flow through `pm_brief.py` / `pm-brief.sh`, both deleted in crew 1.0 (PM agent, pulse and journal removed). Redraw it for 1.0's SessionStart brief path, or retire it (deleting a diagram needs the owner's yes).
