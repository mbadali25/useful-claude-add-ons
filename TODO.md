...
- **`test_crew_instructions.py`'s doc walk reads untracked agent worktrees (T-0001 round-3 NIT)** - `plugin/crew/tests/test_crew_instructions.py:695` (`_doc_files`, used by the test at `:703`) walks the whole checkout and skips only `_SKIP_DIRS` (`:692`: `.git`, `node_modules`, `graphify-out`), so untracked `.claude/worktrees/agent-*` copies are scanned as shipped docs. Measured at `c5f4aa62`: 2946 passed / 1 failed in the main checkout, 2947 / 0 in a clean worktree at the same commit. Walk `git ls-files` instead of the filesystem.
- **17 transient `test_completion_audit` ps1 failures in one verify-gate `--all` run** - `plugin/crew/tests/test_completion_audit.py:160` and `:179` (the `sh`/`ps1` parametrized cases). At `c5f4aa62` one gate run reported 17 ps1 failures; a direct full-suite run and a second gate run at the same commit were green, so it did not reproduce. Suspected cause, not proven: the snap-packaged pwsh (`/snap/bin/pwsh`, a symlink to the snap `powershell` launcher on this host) under the gate's concurrency. Next occurrence: keep the failing run's output and the pwsh path the fixtures resolved.
- **`/crew:done` stales its own approval receipt** - `plugin/crew/commands/done.md:48` (step 1) rewrites `spec.md`'s header to `status: done` after check 1 has passed, and step 2 (`:56`, `crew_metrics.py record`) then reads the approval as stale: `plugin/crew/hooks/scripts/crew_ticket.py:508` reports "spec.md changed since approval". Either exclude the status header from the approval hash, or record metrics before the header edit.
- **The plan template asks for `path:line` in `Files:`, which `parse_plan` cannot take** - `plugin/crew/commands/plan.md:33` and `plugin/crew/skills/crew-plan/SKILL.md:39` ask for `path:line` where modifying existing code, but `crew_ticket.parse_plan` (`plugin/crew/hooks/scripts/crew_ticket.py:293`, via `_split_files` at `:287`) takes every backticked token on a `Files:` line as a path, and every whitespace-split word when nothing is backticked. Reproduced at `f2bb919b`: `Files: modify src/a.py` yields the entries `modify` and `src/a.py`, and `src/a.py:42` is not covered by a Touch entry `src/a.py` (a glob such as `src/**` happens to match it), so `:456` reports it outside Touch. Hit by the T-0004, T-0005 and T-0006 planning lanes. Strip a trailing `:line[-line]` before matching, and make the template's own example parse.
- **verify-gate looks for tickets at the 0.20 layout** - `plugin/crew/hooks/scripts/scope_report.py:62` reads only `.work/tickets/<id>.md` and `.work/cache/<id>.md` (`_TICKET_DIRS` at `:41`), so a 1.0 ticket folder (`.work/tickets/<id>/spec.md`) is reported as "ticket file is missing" (`:174`) and the gate's outside-scope report never runs for a 1.0 ticket. Read `<id>/spec.md`'s Touch through `crew_ticket.parse_touch` first.
- **Guards and CLI approval read `.crew/config.json`; `/crew:migrate` writes `.crew/crew.json`** - `plugin/crew/hooks/scripts/crew_state.py:264` (`read_config`), `plugin/crew/hooks/scripts/crew_ticket.py:518` (`scope.allowCliApproval`) and `:684` (`scope.mode`) all read `config.json`, while `plugin/crew/hooks/scripts/crew_migrate.py:473` writes the migrated settings to `crew.json` and keeps `config.json` (table at `:11`). A setting changed in `crew.json` after migration takes effect nowhere. Pick one file as the source the hooks read, or have migrate say which one is live.
- **HTTP deploys and workspace changes pass `cloud_guard.py` unclassified (deferred from T-0005/T-0009)** - `plugin/crew/hooks/scripts/cloud_guard.py:1131` recognises only `gh pr merge --admin` under `gh`, and nothing handles `curl`. Reproduced at `f2bb919b`: `cloud_guard.scan("bash", ...)` returns no finding for `gh api -X POST repos/o/r/actions/workflows/deploy.yml/dispatches`, `gh run rerun 12345`, `gh workflow run deploy.yml`, `gh api -X DELETE repos/o/r`, or a `curl -X DELETE`/`-X POST` to `app.terraform.io/api/v2/...` (TFC/HCP workspace delete and run create). Needs a URL-and-method parser whose `unknown` environment is never allowed unattended, plus a must-block/must-allow suite, sabotage-tested, per CLAUDE.md's rule for a blocking hook.
...
- **`sabotage.py` ends FAIL on Linux with the same 14 non-red entries** - `python3 plugin/crew/tests/sabotage.py` ends `SABOTAGE SUITE: FAIL`; its verdict loop (`plugin/crew/tests/sabotage.py:3237`, `:3246`) checks only the exit code, with no SKIPPED-vs-PASSED distinction. Identical list at `c5f4aa62`, at T-0003's `cf3bc0d8`, and re-run for this entry at `f2bb919b`: 13 STILL GREEN, 1 RED BUT UNPROVEN (exit 4). Most target PowerShell-gate tests that skip off Windows. The Get-CrewChildTabRecheck entry is already filed above ("sabotage.py's own aggregate run has the same SKIPPED-vs-PASSED gap"); the other 12 STILL GREEN, by name:
  ...
  Plus the RED BUT UNPROVEN one, `docs.theme default goes back to the string neutral` (`plugin/crew/tests/sabotage.py:988`), which exits 4 rather than failing a test. Three of the twelve (`frozen artifact path`, `schema 5 migration`, `bash gate stops publishing a deadline`) have names that do not mention PowerShell, so the SKIPPED-vs-PASSED fix may not explain them; check each one's target test before assuming it does. (The spec counted "the other 13"; by name the list is 12 STILL GREEN plus the exit-4 one, 14 non-red with Get-CrewChildTabRecheck.)
...
- **Two structural-gate docstrings claim to catch an inverted condition (T-0002 review round-2 NIT, accepted)** - `plugin/crew/tests/test_sendkeys_structural_gate.py:292` (`test_the_inhibit_gate_declines_rather_than_proceeds`) and `:308` (`test_foreground_zero_is_treated_as_unknown_not_as_still_focused`) say they catch an inverted condition; they are text checks and do not. Only the pwsh-slice tests at `:401` and `:432` run the decision, and those skip without pwsh (`:368`). (Lines were `:1049`, `:1065`, `:1191` on the T-0002 branch.) Narrow the two docstrings to what they check, or drop them as redundant with the slice tests.
