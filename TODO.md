...
- `plugin/crew/hooks/scripts/_common.sh` `crew_py_strict` (and its byte-copy in `plugin/crew/hooks/scripts/role-write-guard.sh`) still accepts any candidate that prints an executable path; the .ps1 `Resolve-CrewPython` now demands a JSON proof of CPython/PyPy >= 3.8. A Python 3.7, or a liar printing a real path, resolves in bash and not in PowerShell, so the flavours can still disagree on that host. Not done in round 2: dozens of fixtures in `plugin/crew/tests/test_verify_gate_python3_shim.py` and `test_context_watch_python_resolver.py` are `echo <path>` stubs that a proof would reject, and the brief scoped the proof to the .ps1 and obsidian-vault's six wrappers.
