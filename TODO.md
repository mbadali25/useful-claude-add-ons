...
- `plugin/crew/tests/test_verify_gate_python3_shim.py:472,584` still hardcode `crew_py_strict`'s PRE-floor proof text (`"import sys; print(sys.executable)"`) in two fixture stubs and three doc comments (`:253,466,700`). `_common.sh`'s `crew_py_strict` (and `role-write-guard.sh`'s byte-copy) now sends `"import sys; sys.version_info>=(3,8) and print(sys.executable)"` (burn-in fix3b). Currently harmless here (10/10 pass, 1 skip) because these fixtures fall through to a real system python rather than asserting on the mismatch the way `run-tests.sh`'s UNRUNNABLE case did (item B) - but the same staleness, and the same silent fallback risk if a future edit removes the safety net that currently masks it.
