...
- CLOSED 2026-09-25 (T-0007, crew 1.0.29; fix by win-repo-2, cherry-picked from
  ...
  `plugin/crew/tests/crew_fixtures.py:667` (`resolve_bash`) prefers Git for
...
- OPEN 2026-09-25 (found by T-0007's enumeration, deferred - outside its Touch
  list): sh tests that put a stub dir on PATH and may exercise the REAL binary
  on native Windows, because `resolve_bash()` is Git's `bin/bash.exe`, which
  prepends `/mingw64/bin:/usr/bin` ahead of the stub dir. Enumerated with
  `grep -nE 'PATH=.*(shell_path\("sh"|os\.pathsep)'` plus the
  `env["PATH"] = ...` form over `plugin/crew/tests/*.py`; every other hit
  either runs through pwsh or native python only, is Windows-skipped, or stubs
  a name Git does not ship (python*, tmux, xdotool, wtype, npm, codex).
  Linux cannot show any of these; each needs a native run.
  - `plugin/crew/tests/test_auto_clear_review_fixes.py:307`
    (`test_context_watch_forwards_the_notify_json_even_when_mktemp_fails`,
    stub from `_no_mktemp_path` at `:282`): stubs `mktemp` to always fail;
    Git's `/usr/bin/mktemp` wins, and nothing proves the stub ran - the
    assertions (stdout JSON, `sent - method notify`) hold with the real
    `mktemp` too. Can pass silently while testing nothing. Fix shape: T-0007's
    `resolve_bash_no_prepend()` on Windows plus a stub counter asserted first.
  - `plugin/crew/tests/test_auto_cycle.py:1021`
    (`test_the_configured_delay_reaches_the_detached_senders_own_sleep_argument`):
    stubs `bash`, and Git ships `/usr/bin/bash`, so the stub likely loses. It
    asserts `sleep_log.exists()`, so this should fail loudly on Windows, not
    pass silently - unmeasured.
  - `plugin/crew/tests/test_auto_cycle.py:313`
    (`test_context_watch_stdout_reaches_eof_promptly_even_with_a_long_delay`):
    stubs `tmux`, which Git does not ship, so the prepend should not defeat
    it; listed because review round 1 named it. Its only proof the stub ran
    is the `sent - method tmux` log line.
  - Inside T-0007 but narrower than it looks:
    `plugin/crew/tests/test_verify_gate_stop_gate_record.py`'s
    `test_1_crlf_from_native_python_does_not_leak_an_inherited_credential`
    now proves its CRLF `python3` stub ran, but measured on Linux the stub
    only answers `crew_py_strict`'s probe and plain `crew_py` callers
    (`verify_fingerprint.py`, `scope_report.py`). `crew_py_strict` returns the
    real interpreter's `sys.executable`, so the env-pin read the test is named
    for never goes through the stub.
...
**Not covered by T-0007 (crew 1.0.29) - re-scoped 2026-09-25, still OPEN.**
...
- `test_verify_gate_stop_gate_record.py::test_34d_a_second_mktemp_failure_refuses_rather_than_wedges` -
  **root cause now confirmed FIXTURE, not product** (runs 36106846833/36106851305). The test shadows
  ...
  CLOSED 2026-09-25 for `test_34d` (T-0007, crew 1.0.29): the separator was half of it (`f7b70aae`); the
...
- `test_34d_a_second_mktemp_failure_refuses_rather_than_wedges` - `:1646`. sh-flavour only
  ...
  crew 1.0.29): the stub `mktemp` was never reached through Git's `bin/bash.exe` - see the CLOSED entry near
...
- **Spec-template placeholder Touch globs parse as real paths (T-0001 round-3 NIT)** - `plugin/crew/commands/spec.md:31` (`src/area/**`) and `plugin/crew/commands/fix.md:46` (`path/or/glob/**`) are accepted by `crew_ticket.parse_touch` (`plugin/crew/hooks/scripts/crew_ticket.py:267`) as ordinary entries, so a spec that copies either template unchanged passes validation with a Touch naming paths that do not exist. `parse_touch` already refuses a bare angle-bracket placeholder: at `f2bb919b`, a Touch bullet `` `<path/or/glob>` `` returns the problem `Touch: placeholder '<path/or/glob>'`. A mixed form is still accepted: `` `src/<area>/**` `` returns `(['src/<area>/**'], [])`. So switching the templates to a bare `<...>` placeholder would be refused, and a mixed one would not; either use the bare form, or make `parse_touch` refuse any entry containing `<`, and assert the refusal.
- **`test_spec_templates_validate` checks the templates piecewise, not through `validate` (T-0001 round-3 NIT)** - `plugin/crew/tests/test_spec_templates_validate.py:35`, `:46` and `:66` call `crew_ticket.sections` and `crew_ticket.parse_touch` separately and never `crew_ticket.validate` (`plugin/crew/hooks/scripts/crew_ticket.py:427`), so a check later added to `validate` is not held against the templates. Call `validate` on a ticket built from each template.
- **`test_crew_instructions.py`'s doc walk reads untracked agent worktrees (T-0001 round-3 NIT)** - `plugin/crew/tests/test_crew_instructions.py:695` (`_doc_files`, used by the test at `:703`) walks the whole checkout with `os.walk` and skips only `_SKIP_DIRS` (`:692`: `.git`, `node_modules`, `graphify-out`), so untracked `.claude/worktrees/agent-*` copies are scanned as shipped docs. Measured at `c5f4aa62`: 2946 passed / 1 failed in the main checkout, 2947 / 0 in a clean worktree at the same commit. Walk `git ls-files` instead of the filesystem.
- **17 transient ps1 failures in `test_completion_audit.py` in one verify-gate `--all` run** - `plugin/crew/tests/test_completion_audit.py`. At `c5f4aa62` one gate run reported 17 failing ps1 cases in this file; a direct full-suite run and a second gate run at the same commit were green, so it did not reproduce. The failing test IDs were not kept, so which ps1 cases failed is unknown; the file has many ps1-parametrized cases (18 ps1 IDs in its default collection at `f2bb919b`), not one family. Suspected cause, not proven: the snap-packaged pwsh (`/snap/bin/pwsh`, a symlink to the snap `powershell` launcher on this host). Next occurrence: keep the failing run's output, including the test IDs and the pwsh path the fixtures resolved.
- **~~`/crew:done` stales its own approval receipt~~ - CLOSED by T-0026.** The approval receipt now binds a digest that normalises only the spec header's `status:` value (`crew_ticket.approval_digest`), so the step-1 status edit keeps the approval; the option chosen was the first one proposed below. Original entry: `plugin/crew/commands/done.md:48` (step 1) rewrites `spec.md`'s header to `status: done` after check 1 has passed. Step 2 (`:52`, command at `:56`) runs `crew_metrics.py record`, which calls `crew_ticket.status` (`plugin/crew/hooks/scripts/crew_metrics.py:234`, `plugin/crew/hooks/scripts/crew_ticket.py:482`); that compares spec.md's hash with the approval receipt and reports "spec.md changed since approval" (`plugin/crew/hooks/scripts/crew_ticket.py:508`). Either exclude the status header from the approval hash, or record metrics before the header edit.
- **The plan template asks for `path:line` in `Files:`, which `parse_plan` cannot take** - `plugin/crew/commands/plan.md:33` and `plugin/crew/skills/crew-plan/SKILL.md:39` ask for `path:line` where modifying existing code, but `crew_ticket.parse_plan` (`plugin/crew/hooks/scripts/crew_ticket.py:293`, via `_split_files` at `:287`) takes every backticked token on a `Files:` line as a path, and every whitespace-split word when nothing is backticked. Reproduced at `f2bb919b`: `Files: modify src/a.py` yields the entries `modify` and `src/a.py`; and `src/a.py:42` is not covered by a Touch entry `src/a.py` (a glob such as `src/**` happens to match it), so the Touch check at `:456` reports it outside Touch. Hit by the T-0004, T-0005 and T-0006 planning lanes. Strip a trailing `:line[-line]` before matching, and make the template's own example parse.
...
- **`sabotage.py` ends FAIL on Linux with the same 14 non-red entries** - `python3 plugin/crew/tests/sabotage.py` ends `SABOTAGE SUITE: FAIL`; its verdict loop (`plugin/crew/tests/sabotage.py:3237`, `:3246`) checks only the exit code, with no SKIPPED-vs-PASSED distinction. Identical list at `c5f4aa62`, at T-0003's `cf3bc0d8`, and re-run for this entry at `f2bb919b`: 13 STILL GREEN, 1 RED BUT UNPROVEN (exit 4). Most target PowerShell-gate tests that skip off Windows. The Get-CrewChildTabRecheck entry is already filed above ("sabotage.py's own aggregate run has the same SKIPPED-vs-PASSED gap"); the other 12 STILL GREEN, with labels exactly as `sabotage.py` spells them and the line each is defined on:
  ...
  ``the PowerShell gate stops rejecting a `run` entry it cannot represent`` (`:2330`-`:2331`, split across two string literals, so `grep -F` for the whole label misses; the first line's text hits. Not the `:2262` entry "the Stop gate stops rejecting ...", which goes RED);
  `the PowerShell Stop budget charges each command the rule's cost` (`:2469`);
  `the PowerShell deadline read overflows Int32 again` (`:2533`);
  `the bash gate stops publishing a deadline at all` (`:2556`);
  `the PowerShell gate stops publishing a deadline at all` (`:2575`);
  ``the PowerShell budget lets a priced rule defer an `always` command`` (`:2625`);
  `the PowerShell gate stops making an unpriced rule unconditional` (`:2650`);
  `the PowerShell gate charges a mandatory command twice` (`:2727`);
  `the PowerShell gate hoists a mandatory command out of its rule` (`:2739`).
  Plus the RED BUT UNPROVEN one, `docs.theme default goes back to the string neutral` (`:988`), which exits 4 rather than failing a test. Three of the twelve (`frozen artifact path`, `schema 5 migration`, `bash gate stops publishing a deadline`) have labels that do not mention PowerShell, so the SKIPPED-vs-PASSED gap may not explain them; check each one's target test before assuming it does. (The spec counted "the other 13"; by name the list is 12 STILL GREEN plus the exit-4 one, 14 non-red with Get-CrewChildTabRecheck.)
- **crew's hooks write into a repo that never adopted crew** - T-0006 spike, Claude Code 2.1.282, 2026-09-25: in a fresh scratch repo with no `.crew/`, the first manual `/compact`'s SessionStart created `.crew/config.json`, and PreCompact wrote `.work/HANDOFF.md` plus a 758 KB transcript copy under `.crew/transcripts/` (`plugin/crew/hooks/scripts/handoff-write.sh:30`, `:39`). The write sites that let it happen: `plugin/crew/hooks/scripts/crew_platform.py:524` treats a bare `.crew/` directory as a crew repo and `heal_config` (`:191`) then writes a full `config.json`, which opens `plugin/crew/hooks/scripts/handoff-write.sh:9`'s `[ -f .crew/config.json ]` gate. Which hook made the bare `.crew/` first was not traced; `plugin/crew/hooks/scripts/verify-gate.sh:122` (`record_verified`) and `:517` both `mkdir -p .crew` on Stop without checking for a config. Needs a repro in a scratch repo before any fix, then a must-not-write test.
- **One structural-gate docstring claims to catch an inverted condition it does not (T-0002 review round-2 NIT, accepted)** - `plugin/crew/tests/test_sendkeys_structural_gate.py:292` (`test_the_inhibit_gate_declines_rather_than_proceeds`) says it catches "an inverted condition that would send when the var IS set", but it is a text check: it finds the `CREW_AUTOCLEAR_INHIBIT` needle and looks for `exit 0` in the next 300 characters. Sabotaged at `f2bb919b` by rewriting the gate's opening line in `plugin/crew/hooks/scripts/auto-clear.ps1:1007` to an empty `if` body with the decline moved into an `else` (the needle text kept): `:292` stays green, while the pwsh-slice test `test_the_inhibit_gate_exits_before_the_send_when_set` (`:432`) goes red in both parametrizations. The pwsh-slice tests (`:401`, `:432`) skip without pwsh (`:368`). The neighbouring `:308` (`test_foreground_zero_is_treated_as_unknown_not_as_still_focused`) is accurate as written: removing the `$fg -eq 0` disjunct at `auto-clear.ps1:980` turns it red. (Lines were `:1049`, `:1065`, `:1191` on the T-0002 branch.) Narrow the `:292` docstring to what it checks, or drop that test as redundant with `:432`.

## crew 1.0.x: follow-ups from the post-1.0 knowledge refresh (filed 2026-09-25)

Found while re-deriving `.crew/codemap/` and `docs/diagrams/data-flow-crew-config.mmd` at `6c497a14`. None fixed in the refresh; each is its own change.

- **`plugin/crew/hooks/scripts/role_write_guard.py` still carries pm-specific logic** - the `pm` path-scoped role branch (docstring `:22-25`, the `.crew/link` scope discussion around `:110-139`) and a docstring naming removed roles (`crew:pm`, `crew:analyst`, `:62-65`, plus `agents/pm.md:49-53` at `:4`, a file crew 1.0 deleted). Clean it up for the 4-role roster (explorer, researcher, reviewer, security). Guard change: needs the must-block/must-allow suite re-run and a crew version bump.
- **`.crew/config.json` vs `.crew/crew.json` authority is undecided.** `crew_migrate.py` writes `crew.json` and keeps `config.json` (its module table, `:11`); `plugin/crew/hooks/scripts/crew_status.py:62-77` reads `crew.json` first and falls back to `config.json`. Several runtime readers use only `config.json` (`crew_config.py`, `auto-clear.sh`, `plugin/crew/hooks/scripts/crew_autocycle.py:148`), but not every hook does: `plugin/crew/hooks/scripts/crew_context.py:121-132` (`load_crew_config`, called at `:147`, `:835`, `:975`) reads `crew.json` first, so the crew-context hooks and `plugin/crew/hooks/scripts/handoff-read.sh:41` (`memory.inject`, through `inject_enabled`) follow `crew.json`. The split is per reader, not per file - see the "Hooks and CLI approval read `.crew/config.json`" entry above for the readers that ignore `crew.json`. Needs an owner decision on which file is authoritative before either side changes.
- **`INSTALLATION.md` "Optional: MCP servers" (`:211`, bullet `:217`) still documents a standalone "Playwright" MCP menu row** (`claude mcp add playwright -- npx @playwright/mcp@latest`), and the step summaries at `:76` and `:96` still list "AWS, Azure, Playwright". crew 1.0 replaced that row with `web-testing` (`scripts/install-prerequisites.sh:1211` menu key, `:1229` its label). Rewrite the section for the `web-testing` row; check `INSTALLATION.md` against the `.ps1` twin too.
- **`plugin/localgpu/commands/crew.md`'s role table (`:99-107`) names crew roles removed in 1.0** (`scribe`, `docs-writer`, `analyst`, `planner`, `developer`, `qa-reviewer`, `dba`, `infrastructure-architect`, `smoke-author`, `browser-tester`, `pm`). Rewrite it for the 4-role roster. This is localgpu plugin content: bump localgpu's version (both `plugin.json` and `pyproject.toml`) in the same change.
- **Accepted decisions in `docs/review/*` were never promoted to `docs/adr/`** - e.g. the Obsidian retirement row in `docs/review/04c-cross-reviews.md:104` (and the auto-clear retirement at `:19`/`:41`). `docs/adr/` holds three ADRs (0001-0003); promote each accepted review decision to a numbered ADR, or record why it stays a review note.
- ~~(f) `docs/diagrams/process-crew-brief.mmd` is anchored at 60c79407 and still draws the PM brief flow through `pm_brief.py` / `pm-brief.sh`, both deleted in crew 1.0 (PM agent, pulse and journal removed). Redraw it for 1.0's SessionStart brief path, or retire it (deleting a diagram needs the owner's yes).~~ RESOLVED by T-0015 (branch `T-0015-refresh`): redrawn for the 1.0 SessionStart path and anchored at `f2bb919b`.
- **Three lifecycle steps edit a hashed file after approval (T-0015, traced from source, not executed).** The approval receipt hashes `spec.md` and `plan.md` whole (`plugin/crew/hooks/scripts/crew_ticket.py:501-509`), and the completion audit judges nothing in scope unless that status is `approved` (`plugin/crew/hooks/scripts/completion_audit.py:127-136`). But `plugin/crew/commands/plan.md:59` sets `plan.md`'s header to `status: planned` after `/crew:approve`, `plugin/crew/commands/implement.md:94` sets `spec.md` to `status: review` before `/crew:done`, and `plugin/crew/commands/done.md:48` sets `status: done` after the checks. The done.md case was seen live on 2026-09-25 (`crew_metrics record` reported "spec.md changed since approval"); the plan.md and implement.md cases follow from the same hash and were not reproduced. Either exclude the status header from the hash or move status out of the hashed files. Plugin change: needs a crew version bump. Drawn as dashed edges in `docs/diagrams/process-crew-lifecycle.mmd`.
