...
- `plugin/crew/commands/webtest.md:83` and `plugin/crew/skills/stack-web/SKILL.md:72` still advertise the pre-fix `podman run ... -v "$PWD":/work -w /work` line without the `:Z` SELinux relabel `webtest_guard.py`'s own `runtime_line()` now prints (Codex r1 finding 5, fixed in `plugin/crew/hooks/scripts/webtest_guard.py:646`). Left alone here on purpose: a two-line doc edit was tried and reverted in the same change, because it moved `plugin/crew/BUDGETS.md:10`'s Markdown line-count claim out of sync with `plugin/crew/*.md`'s real total, and BUDGETS.md is owned by a different, concurrent lane this ticket was told not to touch.
