...
- **`.crew/config.json` vs `.crew/crew.json` authority is undecided.** The runtime readers (`crew_config.py`, `auto-clear.sh`, `crew_autocycle.py`) use `config.json`; `crew_migrate.py` writes `crew.json` (its module table, `:11`); `crew_status.py` reads `crew.json` (`:63-70`). A migrated repo therefore has status reading one file and every hook reading the other. Needs an owner decision on which file is authoritative before either side changes.
