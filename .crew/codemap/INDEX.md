...
| [`crew.md`](crew.md) | `adf8d1dd` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008) | The `crew` plugin after 1.0: hooks, the four agents, commands, skills inventory, config layering and leaf counts, and how `crew_freshness.py` reads this very directory. |
| [`verification-harness.md`](verification-harness.md) | `adf8d1dd` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008) | `_verify/smoke.sh`, `_verify/run-all.sh`, `scripts/check-marketplace.py`, and `.crew/verify.json` — what each actually runs, and where they overlap or don't. `.crew/verify.json` is tracked; its own `anchor` field (`:3`) is stale at `5238be3d`, independent of this note's anchor. |
...
| [`install-scripts.md`](install-scripts.md) | `adf8d1dd` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008) | The `install-prerequisites.{sh,ps1}` matched pair: catalog parity, the `pick_fit`/`Format-PickerLine` no-bypass rule, idempotency branches, and hook-plugins-default-off on both sides. |
