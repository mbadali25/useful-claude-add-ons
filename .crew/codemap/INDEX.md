...
T-0006 (2026-09-26) moved those five and `localgpu` to `6d35ef8c`, its code commit after
rebasing onto `768a747a`, the same way. T-0008 landed by rebase-merge, so `8d447a7d` is not on
main; its tree matched `768a747a` for every cited path, which each note's last section records.
T-0006's review round 3 moved the same five (not `localgpu`, which nothing it cites changed) to
`2bb92f32`, the round-3 version re-set after its code commits, the same way.

| File | Anchor | Last pass | Covers |
|---|---|---|---|
| [`marketplace-registration.md`](marketplace-registration.md) | `2bb92f32` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008), then to `8d447a7d` (T-0008 review round 3), then to `6d35ef8c` (T-0006), then to `2bb92f32` (T-0006 review round 3) | The marketplace itself: what registers a skill vs. a plugin, the two install scripts, and the two separate version-check paths (`check-marketplace.py` vs. `_verify/smoke.sh`). |
...
| [`crew.md`](crew.md) | `2bb92f32` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008), then to `8d447a7d` (T-0008 review round 3), then to `6d35ef8c` (T-0006), then to `2bb92f32` (T-0006 review round 3) | The `crew` plugin after 1.0: hooks, the four agents, commands, skills inventory, config layering and leaf counts, and how `crew_freshness.py` reads this very directory. |
| [`verification-harness.md`](verification-harness.md) | `2bb92f32` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008), then to `8d447a7d` (T-0008 review round 3), then to `6d35ef8c` (T-0006), then to `2bb92f32` (T-0006 review round 3) | `_verify/smoke.sh`, `_verify/run-all.sh`, `scripts/check-marketplace.py`, and `.crew/verify.json` — what each actually runs, and where they overlap or don't. `.crew/verify.json` is tracked; its own `anchor` field (`:3`) is stale at `5238be3d`, independent of this note's anchor. |
...
| [`install-scripts.md`](install-scripts.md) | `2bb92f32` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008), then to `8d447a7d` (T-0008 review round 3), then to `6d35ef8c` (T-0006), then to `2bb92f32` (T-0006 review round 3) | The `install-prerequisites.{sh,ps1}` matched pair: catalog parity, the `pick_fit`/`Format-PickerLine` no-bypass rule, idempotency branches, and hook-plugins-default-off on both sides. |
...
| [`repo-docs.md`](repo-docs.md) | `2bb92f32` | **re-derived** 2026-09-25 at `6c497a14`; re-anchored to `f2bb919b` (T-0015); per-path re-verify to `adf8d1dd` (T-0008), then to `8d447a7d` (T-0008 review round 3), then to `6d35ef8c` (T-0006), then to `2bb92f32` (T-0006 review round 3) | `docs/` and `CHANGELOG.md`. Records that `docs/adr/` holds three ADRs while accepted decisions under `docs/review/` were never promoted to it, and that TODO.md's `render.sh` entry is still open though the `cygpath -w` fix is in source. |
