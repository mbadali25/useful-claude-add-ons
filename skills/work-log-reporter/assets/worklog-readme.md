...
```
work-log/
├── worklog.config.json          committed — SMTP + reporting settings, no secrets
├── .gitignore                   keeps local state and generated PDFs out of git
├── 2026-07-30-session-01/
│   ├── notes.md                 human-readable log
│   └── session.json             structured source of truth
└── reports/                     generated HTML + PDF (gitignored by default)
```
...
the day, one report goes out covering everything since the last report — which
