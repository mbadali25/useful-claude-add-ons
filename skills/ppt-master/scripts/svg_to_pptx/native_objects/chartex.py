...
def _chart_ex_series_xml(chart_data: dict[str, Any]) -> str:
    ...
    if chart_type in {"sunburst", "treemap"}:
        ...
        return (
            ...
            f'<cx:dataLabels pos="{label_pos}"><cx:visibility seriesName="0" categoryName="1" '
            f'value="1"/></cx:dataLabels>'
            '<cx:dataId val="0"/>'
            f"{layout_pr}"
            "</cx:series>"
        )
...
def _chart_ex_style_xml() -> bytes:
    # A single 9.8 KB OOXML chartStyle part, emitted verbatim. Wrapping it would
    # mean either concatenating fragments, which invites a lost space, or
    # reflowing XML that Office parses byte for byte.
