...
(each skill is standalone; change both). None of the commands below have
...
6. **Record the gate.** Who typed the phrase, when, against which list - on the ticket when
   ticketing is available (run `ticketctl.py redact-check --emit` on the note first), otherwise in
   the closing report. The ticket is advisory; the record is not.
...
- The approver's name is recorded - ticket or closing report (Step 28).
...
Then wait until `Get-MgUser -UserId '{User}'` returns `Request_ResourceNotFound`
...
- `MailboxState = Inactive` (Path 5 is for inactive mailboxes only; a live mailbox's hold is Path 1).
- The address-collision check (Path 5, step 5.0) ran: `Get-Mailbox -Identity '{User}'` returned
  **nothing**, or the written authorisation names the `ExchangeGuid` explicitly. If a live mailbox
  shares the address and the authorisation names only the address, **refuse** - the target is
  ambiguous and `DESTROY 1` would be typed against the wrong understanding.
...
- `ExchangeGuid` recorded, and every command in 5.2-5.3 carries `-InactiveMailbox -Identity '{ExchangeGuid}'`.
  Never `'{User}'` on this path: with a reused address, `Set-Mailbox -Identity '{User}'` modifies
  the live mailbox.
...
>
> Removing the last hold from **{DisplayName}** ({ExchangeGuid}, deleted {WhenSoftDeleted}) starts
> the purge. Clearing the delay hold at step 5.3 turns it into an ordinary soft-deleted mailbox; 30
> days after that it and every item in it are gone. {If a live mailbox shares the address:} A live
> mailbox with the same address exists ({ActiveExchangeGuid}) - it is NOT the target and will not be
> touched, because every command names the inactive ExchangeGuid.
>
