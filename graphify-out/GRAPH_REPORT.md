# Graph Report - uca-t0006-land  (2026-09-26)

## Corpus Check
- 934 files · ~1,941,136 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 60 file(s) not represented in the graph (top: (none) 34, .mmd 8, .csv 4)

## Summary
- 17043 nodes · 33176 edges · 865 communities (704 shown, 161 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 2172 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a0c0847e`
...
## Communities (865 total, 161 thin omitted)
...
Cohesion: 0.08
Nodes (45): Added, Re-anchor provenance - `6d35ef8c` -> `2bb92f32`, 2026-09-26 (T-0006 review round 3), 14a. `resume.auto` — auto-resume after `/clear`, armed by the machine only, _already(), _auto(), _blank(), _compact_was_manual(), decide() (+37 more)
...
Cohesion: 0.06
Nodes (79): _cygpath_list(), `C:\\x\\y` -> `/c/x/y`, the shape `cygpath -u` gives. Pure, so the Windows…, `cygpath -u -p` over the whole list in one process, or None., The PATH value `flavor` ("sh" or "ps1") should be handed, with `dirs` first and…, shell_path(), windows_to_posix(), _crlf_python_env(), by_flavor (+71 more)
...
Cohesion: 0.10
Nodes (8): Task 3: Log surfaces, Task 4: Live diagnostic tools, Subcommands, MerakiClient, Follow Link: rel=next. Never injects a perPage default -- the caps differ per…, TestOrgResolution, TestPagination, TestOtherLogSurfaces
...
Cohesion: 0.08
Nodes (14): Hooks crew ships, Testing, Embedder, memmap, Any, Owns ``vectors.f16`` and ``meta.sqlite`` in one directory., Rows physically present in the vector file, dead ones included., ``(live chunks, tombstoned chunks)``. (+6 more)
...
### Community 30 - "os"
Cohesion: 0.02
Nodes (182): argparse, atexit, Added, errno, filecmp, importlib_util, io, json (+174 more)

### Community 31 - "make_repo"
Cohesion: 0.01
Nodes (221): make_repo(), Write a synthetic crew repo under tmp_path and return its root., Smoke tests for the fixture builder that every later crew test depends on.…, test_a_windows_shim_gets_a_cmd_twin_the_native_which_can_find(), test_commit_with_date_backdates_author_and_committer(), test_head_sha_length(), test_make_repo_default_is_a_real_git_repo(), test_make_repo_git_false_skips_commit() (+213 more)

### Community 32 - "family"
Cohesion: 0.07
Nodes (31): Blast radius, crew: complexity-based dev routing, Decisions taken, Migration (2 -> 3), Problem, Related requirement: crew owns the Copilot model, and sets it up, Related requirement: fallbacks, per-role Codex models, and a Codex advisor, Related requirement: split QA by what it actually checks (+23 more)
...
Cohesion: 0.04
Nodes (92): Added, Re-anchor provenance - `8d447a7d` -> `c35edda5`, 2026-09-25 (T-0034), Re-anchor provenance - `adf8d1dd` -> `8d447a7d`, 2026-09-25 (T-0008 review round 3), The artifact refresh check (T-0008, crew 1.0.36), Re-anchor provenance - 2b337296 -> 5d1fc5fd, 2026-09-22, Re-anchor provenance — 2b337296 -> 5d1fc5fd, 2026-09-22, Landmines, git_out() (+84 more)
...
Cohesion: 0.06
Nodes (49): collections, docx_text_hyperlink, docx_text_paragraph, docx_text_run, build_spec(), _check_hyperlink_style(), _ext_for_part(), _extract_banner() (+41 more)
...
Cohesion: 0.09
Nodes (23): Dark pages, Spec format, _el(), _hanging_indent(), A blank document carrying what a template file would: page size and margins,…, Set w:spacing in twips directly; the Pt-based API rounds these values., Builds one step-by-step SOP .docx on the active brand pack's house template., 1-row/2-col table; the 86-twip cell shaded EF483D is the accent bar. (+15 more)
...
Cohesion: 0.06
Nodes (25): File Structure, Task 1: Transport core, Task 2: Bootstrap, cache, and pagination, Task 5: Semantic diff, default-rule handling, redaction, Task 6: Write CLI — snapshot, apply, rollback, hard blocks, Task 7: Action batches, Task 9: Repo registration, Hard blocks (+17 more)

### Community 49 - "http_with"
Cohesion: 0.09
Nodes (9): Gates that actually exist in code, ConfigTool, Stage a batch with confirmed:false so Meraki validates the whole payload…, Show what the batch will do, get a yes, then commit. A batch can create, update…, http_with(), MerakiHTTP wired to a scripted response queue. Returns (http, calls) where…, TestNetworkLookup, TestRollback (+1 more)
...
Cohesion: 0.05
Nodes (87): Fixed, Entry points, marketplace-registration, Entry points, verification-harness, Calls out to, `check_description_claims` and `check_catalog_claims` — unchanged in shape, `check_self_claims` — three marker types now, not two (+79 more)

### Community 55 - "auth.py"
Cohesion: 0.08
Nodes (37): AuthError, _azure_cli(), _check(), _client_credentials(), decode_claims(), _device_code(), _env(), _find_az() (+29 more)
...
Cohesion: 0.08
Nodes (46): already_configured_global(), apply_migrate_to_repo(), apply_onlyRepos_narrowing(), _atomic_write_json(), check_no_forbidden_words(), convert_method_windows_literal(), describe_global_windows_conversion(), describe_notify() (+38 more)
...
Cohesion: 0.14
Nodes (20): ChunkRecord, One window of one file, before it has a row number., The primitive the search() fix relies on: `limit` bounds which rows are read…, Reproduces the exact bug: before the fix, search() built `matrix` from an N-row…, test_live_rows_limit_excludes_rows_not_yet_trusted(), test_search_does_not_raise_when_a_row_lands_between_matrix_and_metadata(), make_indexer(), paths_in() (+12 more)
...
Cohesion: 0.05
Nodes (83): Auto-resume after `/clear` (T-0006, crew 1.0.40), `.crew/config.json` vs `.crew/crew.json` — the open 1.0.x authority question, acquire_lock(), _agent_task(), anchor_state(), append_log(), build(), bullets() (+75 more)
...
Cohesion: 0.16
Nodes (17): _block_untouched(), _dig(), _keys_added(), _leaf_paths(), _pinned_at_default(), _pop_path(), _prune_unsupplied_global_leaves(), Brings an out-of-date crew setup up to the current schema. Not "v1 -> v2",… (+9 more)
...
### Community 107 - "report_template.py"
Cohesion: 0.13
...
Cohesion: 0.03
Nodes (120): copy, datetime, docx, docx_enum_section, docx_enum_text, docx_opc_constants, docx_oxml, docx_oxml_ns (+112 more)
...
Cohesion: 0.18
Nodes (27): _decide_compact(), _env(), _no_python_path(), _precompact(), needs_bash, needs_pwsh, T-0006 through the real hook wrappers: the PreCompact record both `handoff-…, The sabotage target: a record that says `auto`, read by `decide`. (+19 more)
...
Cohesion: 0.02
Nodes (161): Added, Fixed, crew, Entry points, Config shape, 1. Templates, 10. Global-settable keys — 66, 11. Repo-only keys — all 41 (+153 more)

### Community 129 - "scope_base.py"
Cohesion: 0.05
Nodes (55): Changed, 0. Rebuild vs reshape, 10. Brainstorm → spec → plan → implement, 11. Codex parity, 13. Validation, 14. Scorecard: reaching Ahead, 15. Work guides (`docs/guides/crew/`), 16. Ordered tickets (+47 more)
...
Cohesion: 0.09
Nodes (39): Fixed, Invoking Codex, The self-review, What is allowed to count as a review, build_state(), _cleanup(), cmd_assemble(), cmd_build_prompt() (+31 more)
...
Cohesion: 0.07
Nodes (105): `.crew/verify.json` — 26 rules, up from 25, _approved_paths(), _artifact(), _check(), _codemap(), _codemap_body(), _commit(), _diagram() (+97 more)
...
Cohesion: 0.03
Nodes (109): calendar, Added, Fixed, Fixed, Lessons - each one cost real time here, more than once, Re-anchor provenance - `8d447a7d` -> `6d35ef8c`, 2026-09-26 (T-0006), The roster, replaced wholesale, 12. Migration and version plan (+101 more)
...
Cohesion: 0.14
Nodes (21): claude_hooks(), _codex_bin(), _codex_hooks_generated(), codex_probe(), default_plugin_root(), _hand_written(), _is_generated(), _kept_block() (+13 more)
...
Cohesion: 0.04
Nodes (92): (rounds_list_or_UNKNOWN, source_note)., _review_rounds(), accept(), change(), check_receipt(), check_ticket(), common_dir(), continue_with_successor_plan() (+84 more)
...
### Community 199 - "sys"
Cohesion: 0.01
Nodes (203): builtins, crew_config, ctypes, _no_real_global_config(), fixture, pytest_collection_modifyitems(), Shared test isolation for the whole suite. Every test that touches…, Point every reader of the machine-global config at a path that does not exist,… (+195 more)
...
Cohesion: 0.02
Nodes (158): Added, Entry points, localgpu, config, bootstrap.ps1 vs bootstrap.sh — verdict: the "twin" claim holds, `check_embed_model` — line numbers, re-checked at this anchor, Entry points, Full re-derivation, 84976536 -> 2b337296, 2026-09-22 (+150 more)

### Community 206 - "render_engine.py"
Cohesion: 0.04
Nodes (82): pil, doc-builder's reach, and its degraded paths, pymupdf, Preflight, Renderer resolution - chosen, never fallen back into, convert(), _insert_display_bg(), main() (+74 more)
...
### Community 209 - "_repo"
Cohesion: 0.18
Nodes (22): _context(), _git(), parametrize, The handoff item exactly as crew_context built it before T-0006…, The spike (Claude Code 2.1.282) proved an interactive SessionStart drops…, A crew repo with ticket T-0001, a machine file, and (by default) a handoff…, Review round 2 FIX crew_context.py:822. Staleness came only from whether…, _repo() (+14 more)

### Community 210 - "test_ranking.py"
Cohesion: 0.14
...
### Community 218 - "test_troubleshooting_guide.py"
Cohesion: 0.39
...
### Community 220 - "make_repo"
Cohesion: 0.10
Nodes (21): make_repo(), A git repo with `.crew/config.json` carrying `scope.mode`, a tracked…, fixture, _repo(), fixture, _repo(), fixture, fixture (+13 more)
...
### Community 235 - "crew 1.0.x: findings from 2026-09-25 (T-0001..T-0005 session)"
Cohesion: 0.21
...
### Community 245 - "crew"
Cohesion: 0.15
Nodes (13): Auto-clear, Calls out to, crew, Hooks, Inventory, Owns data, Re-anchor provenance - `6c497a14` -> `f2bb919b`, 2026-09-25 (T-0015), Re-anchor provenance - `8ebbdedc` + `2bb92f32` -> `a0c0847e`, 2026-09-26 (T-0006 landing) (+5 more)
...
Cohesion: 0.11
Nodes (35): configured_mode(), effective_mode(), The worktree's real top-level directory, or None outside git., (data, state): state is 'absent', 'ok' or 'corrupt'., (ticket_or_None, source, broken). `broken` is True when this worktree's active-…, (value, why). `value` is one of MODES; a corrupt config or an unknown value is…, (mode, why) with `auto` resolved to report/block for `ticket`., _read_json() (+27 more)

### Community 299 - "_big_repo"
Cohesion: 0.12
...
Cohesion: 0.12
Nodes (17): _git(), _handoff(), test_approve_is_never_resumed(), test_autopilot_goal_runs_when_installed_and_the_goal_exists(), test_autopilot_not_installed_waits(), test_branch_mismatch_waits(), test_default_machine_path_is_read_when_none_is_given(), test_goal_file_missing_waits() (+9 more)
...
Cohesion: 0.09
Nodes (44): Which ticket is active, functools, getpass, activate(), _active_path(), active_ticket(), approval_path(), approve() (+36 more)

### Community 319 - "Verification harness"
Cohesion: 0.06
Nodes (26): Re-derive provenance, Code map — index, Coverage — and what is still unmapped, Files, How to read these files, Calls out to, Does, Entry points (+18 more)
...
### Community 363 - "crew-ignore-policy.py"
Cohesion: 0.16
...
Cohesion: 0.05
Nodes (88): commit_file(), commit_with_date(), head_sha(), Short HEAD sha of a fixture repo., Stage and commit one path at the current time. The ordinary case.…, Commit one file with both dates forced, simulating a pulled commit. A pull…, _git(), `knowledgeBehind` and `diagramsStale` had no fixpoint. Both compared `anchor ==… (+80 more)

### Community 371 - "_scan_project_trust"
Cohesion: 0.16
...
### Community 373 - "run"
Cohesion: 0.13
Nodes (15): Self-Review, `/crew:config` — see where a value comes from, and set the global file, backup_codemap(), backup_config(), _bump_anchor(), _head(), main(), Upgrade the repo at `root`. `derived` maps subsystem -> graph sections. (+7 more)
...
### Community 381 - "main"
Cohesion: 0.18
Nodes (12): _emit(), _log(), main(), _note_unpinned(), One row to `.crew/guard.log`. Never raises, never creates `.crew/` --…, ONE JSON object on stdout, or nothing. `allow` and None both print no decision;…, Say ONCE per repo that read-only cloud calls are passing unchecked because…, `(data, command, problem)`. `data` None means not ours to judge. `problem` is… (+4 more)
...
### Community 414 - "._live_rows"
Cohesion: 0.20
Nodes (8): _prefix_key(), _process_lock(), Path, Every known file, or only those recorded under the given roots., Live chunks that pass the filters, keyed by vector row. ``limit`` bounds which…, A normalised ``root`` that only matches whole path components., _under(), RLock
...
### Community 434 - "skills-security-ops"
Cohesion: 0.20
...
Cohesion: 0.08
Nodes (24): Agents — 2, Bundled skills — 1, Bundled skills — 3, Commands — 11, Commands — 6, Hooks — none, deliberately, Hooks — the part that runs without being asked, `localgpu` — the GPU in this machine, as a sidecar (+16 more)
...
Cohesion: 0.20
Nodes (21): Calls out to, Landmines, difflib, cfg(), cmd_diff(), cmd_fetch(), cmd_list_backups(), cmd_rollback() (+13 more)
...
### Community 497 - "StoreError"
Cohesion: 0.18
Nodes (8): RuntimeError, The index on disk cannot be used as-is., ``os.replace`` the compacted file in, tolerating a lingering reader. The in-…, StoreError, SIMULATION, not the real cross-process race - this machine is Windows, where…, The residual cross-process gap this lane does not fully close: a search() in a…, test_compact_reports_a_clear_error_instead_of_a_bare_permission_error(), test_search_detects_a_racing_compaction_instead_of_misassigning_rows()
...
Cohesion: 0.04
Nodes (66): Calls out to, Re-anchor provenance - 3167721f -> 1f97e51c, 2026-09-06, Namespace, pathlib, _agent(), _norm(), Dispatched roles run their gates in the foreground. Three subagents ended a…, Collapse runs of whitespace so an assertion survives a rewrap. Copied… (+58 more)
...
### Community 507 - "cmd_apply"
Cohesion: 0.29
Nodes (9): Owns data, cmd_apply(), _cleanup_scratch(), _create_local_scratch_file(), _local_scratch_path(), A per-run scratch path on the LOCAL machine (the one running this script) -…, A per-run scratch path on the REMOTE (manager) host. Must NEVER be built from…, Create an empty scratch file at `path`, mode 0600, before anything writes to… (+1 more)
...
### Community 539 - "context_fixtures.py"
Cohesion: 0.16
...
### Community 543 - "stale"
Cohesion: 0.11
Nodes (18): Fixed, Lifecycle commands, After two review rounds: a successor plan, Amending scope, Approval, At the end of a turn: the completion audit, Modes, Scope and approval (+10 more)
...
### Community 557 - "graph/src/index.ts"
Cohesion: 0.36
...
### Community 581 - "codex"
Cohesion: 0.03
Nodes (56): Changed, Confirming recall reaches your sessions, Measured against the real `vault_ops.py recall`, 2026-09-23, Reading `--stats`, The seeded-note proof, Where recall comes from, Which channel reaches which agent, Generate the files (+48 more)
...
### Community 600 - "test_spec_templates_validate.py"
Cohesion: 0.33
...
Cohesion: 0.09
Nodes (22): brand.json schema, Brand resolution, Build a report, Build a SOP, Regenerating a brand template, Repairing old masters, Self-test, `SopBuilder` API (+14 more)
...
Cohesion: 0.17
Nodes (13): agents(), 1. The verification map, After database changes, `agents` can name any installed subagent, Cost discipline, Every check ships a demonstrated failing control, Find the repo's own conventions first, How to build the rest (+5 more)
...
### Community 645 - "install-scripts"
Cohesion: 0.22
Nodes (9): Calls out to, Does, Entry points, install-scripts, Landmines, Owns data, Re-anchor provenance - `6c497a14` -> `f2bb919b`, 2026-09-25 (T-0015), Re-derive provenance (+1 more)
...
### Community 663 - "codex_trust"
Cohesion: 0.22
Nodes (9): _codex_home(), codex_trust(), _project_trust(), _project_trust_lookup_keys(), Native path spellings Codex tries for `root`, canonical before literal…, Same `(level, matched_key)` contract as `_scan_project_trust`, read from an…, (level, matched_key, malformed). `malformed` is a human-readable reason the…, (state, detail). `state` is one of TRUST_OK / TRUST_CLOSED / TRUST_UNKNOWN --… (+1 more)

### Community 664 - "_head_name"
Cohesion: 0.25
...
### Community 693 - "_absent_global_headline"
Cohesion: 0.22
Nodes (9): _absent_global_headline(), _carried_conflicts(), _config_lines(), global_theme_defeats_migration(), (lines, carried_count) for the Contradictions section. DEFECT 2. This section…, The report's FIRST line when this machine has no global config at all. Mirrors…, True when the machine-global config still carries the old theme default. The…, The config half of the report: what the migration changed, and what it could… (+1 more)
...
### Community 695 - "test_precompact_record_that_cannot_be_removed_is_blanked_in_place"
Cohesion: 0.22
...
### Community 703 - "Remediation & fixing legacy issues"
Cohesion: 0.22
...
### Community 706 - "product_type_for"
Cohesion: 0.36
...
### Community 707 - "Compliance policies and configuration profiles"
Cohesion: 0.22
...
### Community 711 - ".search"
Cohesion: 0.32
Nodes (6): ndarray, normalise(), L2-normalise to float32. A zero vector stays zero rather than becoming NaN., Append chunks and their vectors. Returns the new chunk ids., Brute-force cosine over every live row, best first., test_normalise_leaves_a_zero_vector_alone()

### Community 713 - "_fed_finding"
Cohesion: 0.29
...
### Community 715 - "repo"
Cohesion: 0.06
Nodes (30): Every ladder role at or below `tier`, in ladder order., roles_for_tier(), 1. Show what is in effect, and where each value comes from, 2. Ask, one block at a time, 3. Show the plan, then write, 4. Say what this did not do, Rules that are not negotiable, The machine-global config walkthrough (+22 more)
...
### Community 727 - "_cli"
Cohesion: 0.25
Nodes (8): _cli(), Review NIT :453. The CLI is T-0013's entry point and can run without the…, Review round 3 FIX :450 under real concurrency: several senders that each got…, test_cli_decide_and_record(), test_cli_decide_applies_the_staleness_rule(), test_cli_decide_runs_on_a_fresh_note(), test_cli_exits_zero_on_garbage(), test_concurrent_records_of_one_decision_let_exactly_one_through()

### Community 728 - "test_record_run_never_overwrites_an_unreadable_resume_state"
Cohesion: 0.25
...
### Community 734 - "_make_stdout_safe"
Cohesion: 0.25
...
### Community 742 - "Reconciling the codemap against the graph"
Cohesion: 0.29
...
### Community 752 - "Inventory and status"
Cohesion: 0.29
...
### Community 761 - "_emitted_config_keys"
Cohesion: 0.40
...
### Community 769 - "24. The emergency lane"
Cohesion: 0.33
...
### Community 772 - "_context_py"
Cohesion: 0.40
Nodes (6): _base_scripts(), _context_py(), _golden_repo(), additionalContext from `scripts`/crew_context.py for one SessionStart., test_unarmed_output_is_byte_identical_to_base(), test_unarmed_output_matches_base_code_live()
...
### Community 791 - "test_16_a_skip_after_a_pass_deletes_the_stale_fingerprint"
Cohesion: 0.40
...
### Community 794 - "_codex_home_empty"
Cohesion: 0.50
...
### Community 796 - "test_every_resume_sabotage_anchor_is_present_exactly_once"
Cohesion: 0.33
...
## Knowledge Gaps
- **2674 isolated node(s):** `ci-status.sh script`, `name`, `private`, `description`, `workspaces` (+2669 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 7542 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **161 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `make_repo()` connect `make_repo` to `test_auto_clear_order.py`, `_fake`, `test_auto_clear.py`, `parametrize`, `test_context_watch_python_resolver.py`, `test_provider_table.py`, `test_incident.py`, `test_refresh_check.py`, `test_context_watch.py`, `test_upgrade.py`, `test_crew_config.py`, `test_a_config_that_is_not_there_reads_absent_on_every_platform`, `test_auto_clear_review_fixes.py`, `_Stdin`, `test_role_write_guard.py`, `[Unreleased]`, `test_the_cli_announces_the_new_key_and_does_not_only_write_it_to_a_file`, `crew Project Manager, Graph-Backed Onboarding, and v1 Upgrade — Implementation Plan`, `make_repo`, `sys`, `test_endpoints.py`, `test_verify_absent_and_diagram_kind.py`, `_repo`, `test_platform_sync.py`, `test_auto_cycle.py`, `head_sha`?**
  _High betweenness centrality (0.015) - this node is a cross-community bridge._
- **Why does `3. Auth for the MCP servers` connect `.device` to `Remaining setup — the steps only you can do`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Why does `codex()` connect `codex` to `family`, `crew_config.py`, `Setup phases`, `read_text`, `install-scripts`, ``obsidian-vault` — one or more Obsidian vaults as Claude Code memory`, `rule-of-two`, `repo`, `crew_instructions.py`, `localgpu`, `crew`, `agents`, `dedupe`, `rule_of_two.py`, `upgrade.md`, `Providers`, `_make_stdout_safe`?**
