# Graph Report - uca-refresh  (2026-09-25)

## Corpus Check
- 910 files · ~1,894,686 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 59 file(s) not represented in the graph (top: (none) 34, .mmd 7, .csv 4)

## Summary
- 16233 nodes · 31233 edges · 814 communities (656 shown, 158 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 2115 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `6c497a14`
...
1. `make_repo()` - 508 edges
2. `TODO` - 88 edges
3. `[Unreleased]` - 86 edges
4. `ready()` - 73 edges
5. `ok()` - 69 edges
6. `make_repo()` - 65 edges
7. `make_ticket()` - 64 edges
8. `_repo()` - 58 edges
9. `handle()` - 55 edges
10. `base()` - 54 edges
...
- `2. Where Codex is wrong` --references--> `_classify_shell()`  [INFERRED]
  docs/review/04c-cross-reviews.md → plugin/crew/hooks/scripts/crew_guards.py
- `0. Rebuild vs reshape` --references--> `_classify_aws()`  [INFERRED]
  docs/review/04a-redesign-fable.md → plugin/crew/hooks/scripts/crew_guards.py

## Import Cycles
- None detected.

## Communities (814 total, 158 thin omitted)
...
Cohesion: 0.05
Nodes (107): email_message, email_utils, EmailMessage, html, _appendix(), _authorized_html(), _card(), _cover() (+99 more)
...
Cohesion: 0.11
Nodes (45): Fixed, Unverified, Descriptive text is unchecked — and the drift it allowed has been repaired, Re-anchor provenance — 2b337296 -> 5d1fc5fd, 2026-09-22, Re-anchor provenance - a573ca24 -> 0a9d8937, 2026-09-14, Two catalogs the marketplace does not govern, What the checker reads — and the two things it does not, What was not re-verified (+37 more)
...
Cohesion: 0.05
Nodes (55): 5.1-only cases (hard blocks on 7, not preferences), LSP, Pitfalls that cost time - 5.1, Pitfalls that cost time - 7, Security hardening (both editions), Stack: PowerShell (5.1 and 7), Verification, verify.json rules to propose (5.1 and 7 checked separately) (+47 more)

### Community 18 - "test_provider_table.py"
Cohesion: 0.01
Nodes (207): _all_cli(), _gone(), _no_cli(), _no_machine_global(), fixture, parametrize, Tests for schema 3's per-role provider table and the self-review guard. The…, Codex finding 1, Critical. The history filter is `item["branch"] == here`. When… (+199 more)

### Community 19 - "gizmoduck.py"
Cohesion: 0.05
Nodes (74): Re-anchor provenance, Gizmoduck Multi-Scanner Routine Implementation Plan, Global Constraints, Open items — all three answered (spec §13.11), Per-tool specifics, Self-review notes, Task 15: routine.py — orchestration, Task 16: Coverage table and target grouping — Markdown path (+66 more)
...
Cohesion: 0.08
Nodes (49): Changed, The endpoint ledger and `endpointUnscanned`, _acquire_endpoints_lock(), _artifact_confirms_scan(), _candidate_record(), declare_endpoint(), _empty_endpoint_doc(), _endpoint_needle() (+41 more)
...
Cohesion: 0.06
Nodes (46): Out of scope for this note, errno, io, AuthError, _azure_cli(), _check(), _client_credentials(), decode_claims() (+38 more)

### Community 31 - "make_repo"
Cohesion: 0.01
Nodes (203): make_repo(), Write a synthetic crew repo under tmp_path and return its root., The third `[:7]` pair, and the honest result: it is NOT pinned here.…, The distinction the code comment calls out explicitly: the question is asked of…, test_an_untracked_report_on_disk_does_not_count(), test_report_tracked_is_false_when_there_is_no_report(), test_the_graph_site_is_documented_as_unpinned(), Smoke tests for the fixture builder that every later crew test depends on.… (+195 more)

### Community 32 - "crew_state.py"
Cohesion: 0.05
Nodes (75): calendar, Added, Fixed, Calls out to, _adopt_slot(), _append_dispatch(), archive_stale_handoff(), _archived_handoff_name() (+67 more)
...
Cohesion: 0.05
Nodes (67): Added, Added, Memory - where the code map and runbooks live, Citation freshness, `context-watch` fails closed once, not silently, crew, `crew_py` is no longer the whole story — `crew_py_strict` exists, `crew_state.py`'s two splits — `crew_freshness.py` and `crew_guards.py` (+59 more)
...
Cohesion: 0.06
Nodes (52): collect_rest_settings(), community_plugins_path(), _declared_default_entry(), default_vault_name(), detect_vault_from_app(), discover_vaults(), identity_check(), _linux_obsidian_running() (+44 more)
...
Cohesion: 0.09
Nodes (33): digest(), sha256 of the bytes on disk. Taken once per target before the first mutation…, The sabotage harness edits real source in place, so its restore is a gate.…, `_restore_all` must not clear `_LIVE` wholesale. A signal-path restore that…, The signal and atexit paths verify too, not just the loop's `finally`.…, Defect 4. The startup guard treats any `.bak` as the only good copy and tells…, A `.bak.partial` is litter, not evidence: the target was never touched, which…, Defect 1 and 2 together, at the level a user meets them: the run that would… (+25 more)

### Community 50 - "argparse"
Cohesion: 0.04
Nodes (55): argparse, datetime, docx, docx_enum_section, docx_enum_text, docx_opc_constants, docx_oxml, docx_oxml_ns (+47 more)
...
### Community 54 - "Corrected at 5d1fc5fd — the fifth inversion is fixed, and two checks are new"
Cohesion: 0.15
Nodes (29): Landmines, Re-anchor provenance - 2b337296 -> 5d1fc5fd, 2026-09-22, Corrected at 5d1fc5fd — the fifth inversion is fixed, and two checks are new, Counts: measure, do not read them here, Entry points, Re-anchor provenance - f9bb78a6 -> 84976536, 2026-09-22, Two version-check paths, not one, Corrected at 5d1fc5fd (+21 more)
...
Cohesion: 0.06
Nodes (47): bootstrap.ps1 vs bootstrap.sh — verdict: the "twin" claim holds, Calls out to, Full re-derivation, 84976536 -> 2b337296, 2026-09-22, localgpu, `.mcp.json` — the gap closed at 0.1.18, QA block, fixed in place, same day, Re-anchor provenance - 1f97e51c -> 84976536, 2026-09-22, Re-anchor provenance - 3167721f -> 1f97e51c, 2026-09-06 (+39 more)
...
### Community 72 - "test_proxy_translation.py"
Cohesion: 0.04
...
### Community 73 - "test_scope_base.py"
Cohesion: 0.07
...
### Community 74 - "test_embed_model.py"
Cohesion: 0.10
...
Cohesion: 0.02
Nodes (115): _git(), _fake_home(), Tests for the endpoint ledger, gizmoduck detection, and endpointUnscanned.…, Not just "never reopens" -- re-declaring must not disturb status at all, in…, Same fix as above, checked portably: on Windows a raw backslash string already…, Reproduced: a ledger entry frozen with native separators -- what freezing on…, Reproduced without the fix: 20 in-process threads declaring 20 distinct…, `"false"` (a JSON string) is truthy in Python; `if value` would read it as… (+107 more)
...
Cohesion: 0.05
Nodes (76): dict_or_empty(), `value` when it is genuinely a dict, else `{}`. `(cfg.get(k) or {})` is the…, acquire_lock(), _agent_task(), anchor_state(), append_log(), _archived_as_stale(), build() (+68 more)
...
Cohesion: 0.03
Nodes (78): dataclasses, Task 14: routine.py — manifest parsing and the authorization gate, Task 18: Cross-tool merge for deps and iac, _deps_key(), _iac_key(), merge_category(), _merge_group(), _native_rule_id() (+70 more)
...
Cohesion: 0.13
Nodes (38): Fixed, commit_with_date(), head_sha(), Short HEAD sha of a fixture repo., Commit one file with both dates forced, simulating a pulled commit. A pull…, The same truncation, at the second site. `read_diagrams` carries its own copy…, 7, 8 and 40-character anchors all match a HEAD that IS that commit. Change…, test_codemap_anchors_match_at_any_length() (+30 more)
...
Cohesion: 0.06
Nodes (61): Added, Added, contextlib, append(), capture(), inbox_path(), InboxLocked, load_offset() (+53 more)
...
Cohesion: 0.06
Nodes (61): When the configured theme does not resolve, build_from_spec(), main(), --out, else the spec's "output" (relative to the spec file), else…, Build a SOP from a spec dict. Relative image/output paths resolve against…, resolve_output(), main(), main() (+53 more)
...
### Community 128 - "plan_global_write"
Cohesion: 0.04
Nodes (92): Fixed, Config, and what a machine-global file may supply, Entry points, `pm.authority`, Re-derivation provenance — ea8a014 -> 84976536, 2026-09-22, Config shape, Migration (2 -> 3), 1. Templates (+84 more)
...
Cohesion: 0.08
Nodes (54): Fixed, completion_audit, Does this bash actually run a script living at a Windows path? This is the…, A bash that can run a script at a Windows path, or None. Returns None rather…, resolve_bash(), _usable(), stop(), _audit() (+46 more)
...
Cohesion: 0.07
Nodes (49): plugin_crew_hooks_scripts_completion_audit, plugin_crew_hooks_scripts_crew_context, active_time_seconds(), append_row(), baseline(), check_ticket(), compare(), effective_ticket_metrics() (+41 more)
...
### Community 135 - "crew_autoclear_setup.py"
Cohesion: 0.07
...
Cohesion: 0.11
Nodes (27): Dark pages, cards(), column_widths(), convert(), data_table(), esc(), _insert_display_bg(), main() (+19 more)
...
### Community 153 - "Marketplace and registration"
Cohesion: 0.22
Nodes (9): A worked example: adding an agent is not a registration, Calls out to, Marketplace and registration, Owns data, Re-anchor provenance - 0a9d8937 -> 975480b7, 2026-09-14, Re-anchor provenance — 84976536 -> 2b337296, 2026-09-22, Re-anchor provenance - 975480b7 -> f9bb78a6, 2026-09-14, The gate's own invocation is now inside git's reach (+1 more)
...
### Community 169 - "collect"
Cohesion: 0.07
Nodes (30): Fixed, Commands - build, test, verify, regression, promote, Landmines - every one of these has already shipped broken, Lessons - each one cost real time here, more than once, Promotion: development -> qa -> production, Reporting - errors verbatim, say what you did NOT verify, Scope discipline - fix the ticket, not what you notice nearby, Stop and ask - the conditions that should halt work (+22 more)
...
### Community 172 - "crew_ticket.py"
Cohesion: 0.08
Nodes (51): functools, getpass, activate(), _active_path(), active_ticket(), approval_path(), approve(), check_ticket() (+43 more)

### Community 173 - "crew_instructions.py"
Cohesion: 0.06
Nodes (50): _array_continuation_open(), _array_line_state(), _array_open_tail(), claude_hooks(), _codex_bin(), _codex_home(), codex_probe(), codex_trust() (+42 more)
...
Cohesion: 0.04
Nodes (83): Added, The ratchet registry — ten keys, not nine, The role-write guard — new since this note's previous anchor, Decisions, Guardrail overrides and the merge-gate workflow — design, Invariants the builders must keep, Open — settle and record in the change, The ask (+75 more)
...
### Community 183 - "test_review_ledger.py"
Cohesion: 0.07
...
Cohesion: 0.07
Nodes (36): Calls out to, docs/adr/ now exists — replaces the previous "does not exist" landmine, Does, Entry points, Older provenance, kept as an account of process rather than a current claim, Owns data, Re-anchor provenance — 03b19262 -> 5d1fc5fd, 2026-09-22, Re-anchor provenance — 0a9d8937 -> 975480b7, 2026-09-14 (+28 more)

### Community 195 - "family"
Cohesion: 0.05
Nodes (54): Added, Blast radius, crew: complexity-based dev routing, Decisions taken, Problem, Related requirement: crew owns the Copilot model, and sets it up, Related requirement: fallbacks, per-role Codex models, and a Codex advisor, Related requirement: split QA by what it actually checks (+46 more)
...
Cohesion: 0.06
Nodes (54): Added, `check_embed_model` — line numbers, re-checked at this anchor, Entry points, Two independent process trees, one shared Ollama, mcp_server_mcpserver, 3. The environment exists, and the `localgpu` CLI is in it, Step 1 — say what is about to happen, then wait for `--full`, Step 2 — hold one model, and only one (+46 more)
...
### Community 218 - "meraki_config.py"
Cohesion: 0.10
...
### Community 222 - "_write_marker"
Cohesion: 0.11
Nodes (45): A fake executable called `name` in `directory`; returns its path. POSIX: one…, `{"PATH": ...}` with `bindir` first, built for `flavor`, plus `extra`., shim_env(), write_shim(), _log(), _machine(), skipif, requirement: `sendkeys` must resolve the owning window, then decline rather… (+37 more)
...
Cohesion: 0.06
Nodes (40): Changed, csv, hashlib, _corrupt_cache(), _file_digest(), fingerprint(), _gate_owned(), _head() (+32 more)
...
### Community 235 - "test_verify_gate_baseline_recording.py"
Cohesion: 0.24
...
### Community 247 - "Systematic debugging"
Cohesion: 0.11
...
Cohesion: 0.09
Nodes (44): Owns data, config_path(), _home(), The user's home directory, HOME-first. os.path.expanduser("~") reads…, ~/.claude/obsidian/config.json, or OBSIDIAN_VAULT_CONFIG when set. The override…, Serialize first, then replace atomically. `open(path, "w")` truncates before…, read_config(), write_config() (+36 more)
...
### Community 253 - "test_vault_profiles.py"
Cohesion: 0.19
...
Cohesion: 0.15
Nodes (20): Calls out to, Does, Entry points, Landmines, Measured this pass, obsidian-vault, Re-anchor provenance, Re-anchor provenance — 5d1fc5fd -> 60c79407, 2026-09-22 (PR #210, "Windows audit wave 3") (+12 more)
...
### Community 262 - "mcp-preflight-catalog.sh"
Cohesion: 0.18
Nodes (12): check(), green(), mkfixture(), mkrealbin(), mkskill(), only_skill(), red(), run_mcp() (+4 more)

### Community 263 - "crew/hooks/scripts/_test/test_flavour_guard.py"
Cohesion: 0.19
...
### Community 275 - "The full rule set, by area"
Cohesion: 0.11
...
Cohesion: 0.04
Nodes (67): pathlib, _no_real_global_config(), fixture, pytest_collection_modifyitems(), Shared test isolation for the whole suite. Every test that touches…, Point every reader of the machine-global config at a path that does not exist,…, Deselect `slow` unless asked for, by `--run-slow` or by any `-m` expression…, _agent() (+59 more)
...
Cohesion: 0.08
Nodes (39): base64, Cloud guard false positives, _approval_is_live(), _aws_destructive(), _aws_identity(), _aws_words(), _az_default_subscription(), _az_destructive() (+31 more)
...
### Community 293 - "install-prerequisites.sh script"
Cohesion: 0.17
...
Cohesion: 0.12
Nodes (24): plugin_obsidian_vault_hooks_scripts_vault_guard, count_notes(), frontmatter_signal(), gather_evidence(), graph_layout_signal(), graphify_manifest(), _holds_notes(), _link_target() (+16 more)
...
Cohesion: 0.11
Nodes (34): accepted(), configured_mode(), effective_mode(), The worktree's real top-level directory, or None outside git., `status`, with an `approved` receipt that did not come from the user's prompt…, (ticket_or_None, source, broken). `broken` is True when this worktree's active-…, (value, why). `value` is one of MODES; a corrupt config or an unknown value is…, (mode, why) with `auto` resolved to report/block for `ticket`. (+26 more)
...
### Community 304 - "test_map_audit_python_family.py"
Cohesion: 0.23
...
### Community 306 - "test_auto_cycle.py"
Cohesion: 0.14
...
Cohesion: 0.03
Nodes (62): [2026-07-28], Added, Added, Added, Added, Added, Added, Added (+54 more)

### Community 318 - "test_normalize.py"
Cohesion: 0.22
...
Cohesion: 0.10
Nodes (13): Code map — index, Coverage — and what is still unmapped, Files, How to read these files, Calls out to, Corrected at this anchor, Does, Entry points (+5 more)
...
Cohesion: 0.10
Nodes (30): _answer(), _crash_mid_claim(), _generation_files(), _handoff_repo_with_broken_path(), Windows burn-in FAIL 3, Codex round 1: the per-event claim, the bash resolvers…, Codex r1 finding 1 (event_claim.py:230): a claimant that loses the O_EXCL race…, Documented, not a defect: nothing in a Notification payload tells the other…, The review's reproduction through the CLI: the winner is 'paused' -- it got… (+22 more)
...
Cohesion: 0.24
Nodes (19): Re-anchor provenance — 2b337296 -> 5d1fc5fd, 2026-09-22, _baseline(), _git(), parametrize, A FAILED turn must still sync the rules that passed - and must not persist the…, MUST-RECORD. A turn with one passing rule and one failing rule must still…, MUST-NOT-ADVANCE. Recording per-rule evidence on a FAILED turn must never let…, Existing behaviour, unchanged by the reorder: nothing failed, nothing was… (+11 more)
...
### Community 353 - "_function_raw_source"
Cohesion: 0.11
...
### Community 354 - "meraki_http.py"
Cohesion: 0.13
Nodes (13): random, _MethodPreservingRedirectHandler, meraki_http.py -- transport layer for the Cisco Meraki Dashboard API v1. Stdlib…, Remove the API key from any string bound for output., Follow redirects without changing the method or losing auth headers., redact(), Puts the skill's scripts/ directory on sys.path so tests can import it., Shared fixtures for the Meraki skill's tests. http_with() fakes the injectable… (+5 more)
...
### Community 357 - "test_autoclear_setup.py"
Cohesion: 0.12
...
### Community 363 - "parametrize"
Cohesion: 0.09
Nodes (23): _child_heredoc_source(), _ps1_source(), Structural (no-execution) regression for Get-CrewChildTabRecheck.…, The detached sender is a separate process, built as a literal here-string and…, test_get_crew_child_tab_recheck_cannot_short_circuit_to_send(), _extract_ps1_function(), _fill(), _pwsh_can_resolve_an_owner_window() (+15 more)
...
### Community 367 - "cloudflare_client.py"
Cohesion: 0.07
Nodes (26): playwright_sync_api, Cloudflare v4 API client + CLI. Self-contained, standard library only (urllib)…, main(), _parse_params(), Drata Public API client + CLI. Handles both auth methods (long-lived API key OR…, telegram_get_chat_id.py - find your chat_id so notify.py knows where to…, main(), parse_viewports() (+18 more)
...
### Community 384 - "_write_crew_json"
Cohesion: 0.12
Nodes (19): The double-invocation hazard the function's own docstring warns about: after…, Reproduces the review finding directly: `crew_migrate.py --apply` copies…, The other direction: BOTH files need converting, but from DIFFERENT starting…, FIX: an `os.replace` failure during the COMMIT loop (both files already staged)…, test_apply_migrate_to_repo_commit_failure_leaves_no_orphaned_temp_file(), test_apply_migrate_to_repo_converts_config_json_the_senders_actually_read(), test_apply_migrate_to_repo_converts_each_divergent_file_from_its_own_content(), test_apply_migrate_to_repo_does_not_widen_without_yes_widen() (+11 more)
...
### Community 390 - "test_auto_clear_order.py"
Cohesion: 0.28
...
### Community 400 - "Auto wrap-up, auto-clear and auto-resume"
Cohesion: 0.12
...
### Community 401 - "Scope and approval"
Cohesion: 0.11
Nodes (18): After two review rounds: a successor plan, Amending scope, Approval, At the end of a turn: the completion audit, Modes, Scope and approval, The contract, Which ticket is active (+10 more)
...
### Community 404 - "test_lifecycle_commands.py"
Cohesion: 0.24
...
### Community 407 - "Crew redesign — Claude Fable 5.1 design"
Cohesion: 0.12
...
Cohesion: 0.23
Nodes (14): audit(), changed_paths(), _git_fields(), main(), _payload(), physical(), Stop-time completion scope audit, and `/crew:done`'s scope check. crew 1.0,…, `path` with every non-printable character escaped, so a filename can never add… (+6 more)
...
### Community 418 - "_fed_finding"
Cohesion: 0.15
Nodes (14): _fed_finding(), carries(), seen(), _head_name(), `(has_c, positional)` for a POSIX shell's argv. Options end at the first non-…, The finding for a destructive-capable tool run by `xargs`/`parallel`, whose…, What a pipeline stage feeds the next one, as far as crew can tell. A literal…, Every placeholder an `xargs`/`parallel` invocation substitutes into the command… (+6 more)
...
### Community 421 - ".verify"
Cohesion: 0.05
Nodes (42): Check 1 — the review receipt, Check 2 — the verify gate, Check 3 — the completion audit, On all three passing, After, `--dry-run`, Nothing changes without a yes at that moment, Relay the script's outcomes; do not re-summarise them (+34 more)
...
### Community 423 - "KnowBe4 Reporting API (read-only)"
Cohesion: 0.17
...
### Community 425 - "2. Credentials and secrets"
Cohesion: 0.14
...
### Community 434 - "make_repo"
Cohesion: 0.14
Nodes (14): make_repo(), A git repo with `.crew/config.json` carrying `scope.mode`, a tracked…, fixture, _repo(), fixture, _repo(), fixture, _repo() (+6 more)
...
### Community 436 - "Scheduling the gardener"
Cohesion: 0.29
...
### Community 449 - "diagram_from_spec.py"
Cohesion: 0.23
...
### Community 456 - "test_review_fallback_bundle.py"
Cohesion: 0.22
...
### Community 458 - "Defense-in-Depth Validation"
Cohesion: 0.17
...
### Community 462 - "os"
Cohesion: 0.02
Nodes (185): builtins, crew_config, filecmp, importlib_util, json, os, _answer_for(), check_answer() (+177 more)
...
### Community 467 - "evaluate"
Cohesion: 0.15
Nodes (12): evaluate(), True when nobody is there to answer an `ask`. `CREW_UNATTENDED` decides when…, The whole judgement for one tool call, as a dict. Pure apart from reading…, unattended(), `--dry-run` sends nothing at all, GitHub branch protection and rulesets, Safety rails, `scripts/merge_gate.sh` (+4 more)
...
Cohesion: 0.11
Nodes (38): Calls out to, Does, Entry points, Landmines, Owns data, Re-anchor provenance, skills-security-ops, key() (+30 more)
...
### Community 473 - "build_gallery.py"
Cohesion: 0.31
...
Cohesion: 0.18
Nodes (10): 1. Do not `@`-file entire documents, 2. Do not write absolute prohibitions, 3. Do not write a comprehensive manual, 4. Do not let it grow unreviewed, Applying this to a repo crew onboards, CLAUDE.md: structure, size, and the four anti-patterns, Size, The four anti-patterns (+2 more)
...
### Community 478 - "test_module_split.py"
Cohesion: 0.21
...
Cohesion: 0.14
Nodes (15): signal, atomic_write(), deep_merge(), Dispatcher, eprint(), main(), _pid_alive(), Path (+7 more)

### Community 480 - "Verification harness"
Cohesion: 0.13
Nodes (15): Calls out to, Entry points, History — what a *previous* task ran (not this one), Owns data, Re-anchor provenance — 0a9d8937 -> 975480b7, 2026-09-14, Re-anchor provenance — 84976536 -> 2b337296, 2026-09-22, Re-anchor provenance — ea8a014 -> 84976536, 2026-09-22, Re-verification pass, 2026-09-12 (+7 more)
...
### Community 486 - "3. Crew departs from three community best practices, on purpose"
Cohesion: 0.22
...
### Community 487 - "_simulate_windows_write_text"
Cohesion: 0.15
...
### Community 488 - "test_rule_of_two.py"
Cohesion: 0.02
...
### Community 500 - "store.py"
Cohesion: 0.06
Nodes (48): MonkeyPatch, numpy, ChunkRecord, RuntimeError, The index on disk: float16 vectors in a flat file, metadata in sqlite.…, Another process already holds the cross-process refresh lock., An OS-level lock so two *processes* cannot both refresh one index.…, The index on disk cannot be used as-is. (+40 more)
...
### Community 504 - "_prune_unsupplied_global_leaves"
Cohesion: 0.20
...
### Community 511 - "The five contradictions the document records about itself"
Cohesion: 0.25
...
### Community 534 - "jq_absence.sh"
Cohesion: 0.52
Nodes (6): drive(), fail(), log_has(), pass(), sent_urls(), jq_absence.sh script
...
### Community 543 - "Setup audit — Claude Code 2.1.280 / Codex 0.155.1, Linux (root)"
Cohesion: 0.22
...
Cohesion: 0.22
Nodes (8): 0. Refuse without an approved plan, 1. Record where this ticket starts, 2. Work the plan's steps in order, 3. Print the changed-file list, every time, even empty, 4. Verify, 5. Specialists, endpoints, coverage, 6. Tests, then docs, then review — in that order, 7. Update status
...
### Community 553 - "test_troubleshooting_guide.py"
Cohesion: 0.39
...
Cohesion: 0.29
Nodes (6): A note on this repository's two licences, MIT License, MIT License, `plugin/crew/skills/crew-brainstorm/`, `crew-plan/`, `crew-execute/`, `plugin/crew/skills/crew-debugging/`, Third-party notices — `crew`
...
### Community 562 - "Pressure Test 1: Emergency Production Fix"
Cohesion: 0.40
...
### Community 566 - "Pressure Test 2: Sunk Cost + Exhaustion"
Cohesion: 0.40
...
### Community 577 - "notify.sh script"
Cohesion: 0.83
Nodes (3): claim_sent(), read_cfg(), notify.sh script

### Community 579 - "verify-gate.sh"
Cohesion: 0.24
...
Cohesion: 0.25
Nodes (7): 0. Debug first if this is a defect, 1. Direction — one line, 2. Spec — six lines, 3. Plan — one step, 4. Implement, tests, docs, 5. Review — one round, 6. Done

### Community 600 - "The Rule of Two — scope"
Cohesion: 0.25
...
### Community 606 - "test_a_config_that_is_not_there_reads_absent_on_every_platform"
Cohesion: 0.50
...
### Community 618 - "25. Command and agent reference"
Cohesion: 0.29
...
### Community 626 - "test_the_cli_announces_the_new_key_and_does_not_only_write_it_to_a_file"
Cohesion: 0.33
...
### Community 636 - "test_an_unusable_store_sends_one_ping_across_both_flavours"
Cohesion: 0.25
...
### Community 644 - "core/src/index.ts"
Cohesion: 0.16
...
### Community 646 - "test_resolver_accepts_a_native_windows_path_to_a_real_target_under_c"
Cohesion: 0.29
...
Cohesion: 0.29
Nodes (6): crew review, September 2026, Measured on this machine, 2026-09-23, Not verified, Verified defects found during the review, Where the three reviews agree, Where they disagree

### Community 648 - "reviewer.md"
Cohesion: 0.29
...
### Community 650 - "parse_plan"
Cohesion: 0.29
Nodes (7): _bullet_entry(), _entry_problem(), parse_plan(), Why `entry` cannot be a scope path, or None., The one path a bullet names, or (None, problem)., (files, counts, problems): every `Files:` entry, the number of Files/Test/Risk…, _split_files()
...
### Community 658 - "Inventory and status"
Cohesion: 0.29
...
### Community 663 - "test_apply_creates_local_scratch_files_before_the_backup_and_cleanup_tracks_only_what_it_created"
Cohesion: 0.29
...
### Community 690 - "end"
Cohesion: 0.25
Nodes (8): Added, Closing, Declaring, Status, What it cannot do, Which of these you were asked for, end(), Close the incident: write the report, archive the state, re-gate. Returns…
...
### Community 692 - "test_the_final_probes_wait_is_capped_to_the_remaining_deadline"
Cohesion: 0.33
...
### Community 707 - "test_cleanup_prints_a_named_warning_when_remote_rm_itself_raises_os_error"
Cohesion: 0.40
...
### Community 709 - "test_thin_wrapper_hooks_do_not_call_the_unhardened_resolver"
Cohesion: 0.50
...
### Community 710 - "_codex_home_empty"
Cohesion: 0.50
...
### Community 720 - "What `.crew/verify.json` actually maps"
Cohesion: 0.67
Nodes (3): One defect fixed since the previous anchor, one still open, `verify-gate.sh` fails closed on no python and on a stub matcher — new since the previous anchor, What `.crew/verify.json` actually maps

### Community 725 - "_no_live_targets"
Cohesion: 0.67
Nodes (3): _no_live_targets(), fixture, `_LIVE` is module state and the handlers are process state. A test that leaves…

### Community 726 - "test_a_marker_written_by_one_flavour_is_honoured_by_the_other"
Cohesion: 0.67
...
### Community 792 - "labtarget_server.py"
Cohesion: 0.31
...
### Community 827 - "vault-capture.sh"
Cohesion: 0.83
Nodes (3): vault-capture.sh script, _vault_capture_reject(), _vault_capture_resolve_python()
...
## Knowledge Gaps
- **2646 isolated node(s):** `ci-status.sh script`, `name`, `private`, `description`, `workspaces` (+2641 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 7256 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **158 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Fixed` connect `GraphClient` to `plan_global_write`, `core/src/index.ts`, `collect`, `test_hook_once.py`, `[Unreleased]`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `Production access — `prodDatabase`, `prodServer`, and `production.*`` connect `crew_config.py` to `plan_global_write`, `crew_guards.py`, `_common.sh`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Why does `crew_incident_log()` connect `_common.sh` to `crew_config.py`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
...
- **Should `worklog.py` be split into smaller, more focused modules?**
