...
## Corpus Check
- 929 files · ~1,916,293 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 60 file(s) not represented in the graph (top: (none) 34, .mmd 8, .csv 4)

## Summary
- 16690 nodes · 32312 edges · 812 communities (659 shown, 153 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 2103 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `adf8d1dd`
...
## Communities (812 total, 153 thin omitted)
...
### Community 1 - "wlconfig.py"
Cohesion: 0.18
Nodes (31): collect_sessions(), config_path(), _deep_merge(), find_root(), list_session_ids(), load_config(), load_state(), new_session() (+23 more)
...
Cohesion: 0.03
Nodes (66): http_server, _cleanup_db(), _init_db(), main(), _handle_sigterm(), labtarget - a deliberately vulnerable, localhost-only web app. Purpose: give…, _bound_handler(), endpoint() (+58 more)
...
### Community 30 - "os"
Cohesion: 0.03
Nodes (107): filecmp, importlib_util, json, os, _junit_outcome(), main(), Mutations for the auto wrap-up / auto-clear / auto-resume cycle. Same tuple…, Read back a `--junitxml` report and say whether the one testcase it collected… (+99 more)

### Community 31 - "make_repo"
Cohesion: 0.01
Nodes (228): make_repo(), Write a synthetic crew repo under tmp_path and return its root., The third `[:7]` pair, and the honest result: it is NOT pinned here.…, The distinction the code comment calls out explicitly: the question is asked of…, test_an_untracked_report_on_disk_does_not_count(), test_report_tracked_is_false_when_there_is_no_report(), test_the_graph_site_is_documented_as_unpinned(), Smoke tests for the fixture builder that every later crew test depends on.… (+220 more)

### Community 32 - "Fixed"
Cohesion: 0.03
Nodes (91): Added, Fixed, Before it ships, Decisions, Open, The load-bearing invariant, The Rule of Two — scope, Verified, and how (+83 more)
...
Cohesion: 0.05
Nodes (81): Added, Owns data, Re-anchor provenance - 2b337296 -> 5d1fc5fd, 2026-09-22, Re-anchor provenance, Re-anchor provenance — 2b337296 -> 5d1fc5fd, 2026-09-22, Landmines, The three readers every crew script shares, and nothing else. Split out of…, Return the file's text, or None if it cannot be read for any reason. utf-8-sig… (+73 more)
...
### Community 38 - "re"
Cohesion: 0.03
Nodes (98): argparse, collections, datetime, docx, docx_enum_section, docx_enum_text, docx_opc_constants, docx_oxml (+90 more)
...
Cohesion: 0.06
Nodes (33): Dark pages, Brand resolution, Build a SOP, Regenerating a brand template, Repairing old masters, Self-test, `SopBuilder` API, Spec format (+25 more)

### Community 46 - "crew_config.py"
Cohesion: 0.04
Nodes (67): Added, copy, 16. `guards` — the configurable guardrails, `block` did not preserve everything, and that is stated rather than implied, `forcePush: allow` honours the value everywhere, and names the branch, `guards.mergeGate` is read by a command, not by the guard, How the two are merged, One resolver, two flavours (+59 more)
...
Cohesion: 0.08
Nodes (64): Fixed, Entry points, marketplace-registration, Entry points, The registration web — skill vs. plugin, re-confirmed at this anchor, Two version-check paths, still not one, Unverified at this anchor, `scripts/check-marketplace.py` — the direct gate (+56 more)
...
Cohesion: 0.07
Nodes (57): 1. The two layers, already_configured_global(), apply_migrate_to_repo(), apply_onlyRepos_narrowing(), _atomic_write_json(), check_no_forbidden_words(), convert_method_windows_literal(), describe_global_windows_conversion() (+49 more)

### Community 63 - "vault_garden.py"
Cohesion: 0.05
Nodes (75): ack(), acked_ids(), add_parsers(), already_distilled(), attributed_files(), _BoundedStreamReader, captured_at(), changed_files() (+67 more)
...
### Community 74 - "_write_handoff"
Cohesion: 0.14
Nodes (24): _log(), _machine(), _proc_comm(), The mirror case: off native Windows (no `OS=Windows_NT`), with no tmux and no…, Wording: the notify path must say what it did (the handoff is written and…, CREW_AUTOCLEAR_INHIBIT exists so a test suite never drives the real keyboard --…, requirement: `sendkeys` must resolve the owning window, then decline rather…, `delay` (not `delaySeconds`) is exactly the shape a hand-edit typo takes:… (+16 more)
...
Cohesion: 0.05
Nodes (80): `.crew/config.json` vs `.crew/crew.json` — the open 1.0.x authority question, dict_or_empty(), git_out(), Stripped stdout of a git command, or None on any failure. Failure includes git…, `value` when it is genuinely a dict, else `{}`. `(cfg.get(k) or {})` is the…, acquire_lock(), _agent_task(), anchor_state() (+72 more)
...
### Community 107 - "Any"
Cohesion: 0.20
Nodes (19): Fixed, _blocks_to_text(), check_fits_context(), estimate_prompt_tokens(), Any, `system` is either a string or a list of content blocks., Flatten one message's content blocks into text Ollama can read., Pull `tool_result` blocks out of a user turn. (+11 more)
...
Cohesion: 0.06
Nodes (53): artifacts(), changed_files(), check_auth_leak(), check_skips(), check_visual(), config_auth_paths(), container_evidence(), declared_storage_state() (+45 more)
...
Cohesion: 0.10
Nodes (31): _approved_paths(), _fenced(), parametrize, T-0008: the artifacts a ticket's changes reach stay current before done.…, An edit that moves a line a mutation aims at would otherwise leave that…, _read(), _run_cli(), _section() (+23 more)
...
Cohesion: 0.03
Nodes (104): Fixed, crew, Entry points, Decisions, Guardrail overrides and the merge-gate workflow — design, Invariants the builders must keep, Open — settle and record in the change, The ask (+96 more)

### Community 129 - "scope_base.py"
Cohesion: 0.09
Nodes (38): `.crew/verify.json` — 24 rules, up from 23, 2. Ticket contract, changed(), _default_ref(), _entry(), _fallback_ref(), _is_ancestor(), _is_commit() (+30 more)

### Community 130 - "test_completion_audit.py"
Cohesion: 0.07
Nodes (65): Fixed, A bash that can run a script at a Windows path, or None. Returns None rather…, resolve_bash(), make_repo(), A git repo with `.crew/config.json` carrying `scope.mode`, a tracked…, stop(), _audit(), _broken_python() (+57 more)
...
### Community 150 - "esc"
Cohesion: 0.12
Nodes (15): brand.json schema, Build a report, cards(), column_widths(), data_table(), esc(), masthead(), meta_table() (+7 more)
...
Cohesion: 0.17
Nodes (33): _artifact(), _codemap_body(), _commit(), _diagram(), _graph(), _graphify(), _no_graphify(), A repo with two source files, a scope base for TICKET at HEAD, and nothing… (+25 more)
...
Cohesion: 0.08
Nodes (50): common_dir(), _allow_cli(), _approve_from_prompt(), _cli(), _exhaust(), _exhaust_after_two(), _plan(), fixture (+42 more)
...
Cohesion: 0.05
Nodes (41): Fixed, Lessons - each one cost real time here, more than once, Entry points, 10. Brainstorm → spec → plan → implement, 11. Codex parity, 12. Migration and version plan, 13. Validation, 14. Scorecard: reaching Ahead (+33 more)
...
Cohesion: 0.02
Nodes (130): Added, Changed, Auto-clear, Calls out to, `.claude/rules/` - generated from this directory, and gated against it, crew, Hooks, Inventory (+122 more)
...
Cohesion: 0.03
Nodes (99): calendar, Added, Config, the guard vocabulary, and the ratchet, 0. Rebuild vs reshape, 9. Stack, hooks, and layer ownership, 17. `change` — change requests, and the ratchet that runs backwards, `requireForProduction` is the sixth, and it ratchets the other way round, The default is `false` and the floor is `true`, and they are not the same (+91 more)
...
Cohesion: 0.07
Nodes (26): File Structure, Task 2: Bootstrap, cache, and pagination, Task 3: Log surfaces, Task 4: Live diagnostic tools, Task 5: Semantic diff, default-rule handling, redaction, Task 7: Action batches, Task 8: SKILL.md and reference documents, Task 9: Repo registration (+18 more)
...
Cohesion: 0.08
Nodes (40): Renderer resolution - chosen, never fallen back into, Convert through Word COM. Windows with Word installed only. Opens whatever Word…, to_word(), locked(), Why `path` cannot be overwritten right now, or None. A viewer holding a PDF…, classify_producer(), conversion_problem(), engine_label() (+32 more)
...
Cohesion: 0.15
Nodes (22): _could_still_be_a_tool_call(), _new_message_id(), _parse_arguments(), An Anthropic Messages API front end for a local Ollama. `ANTHROPIC_BASE_URL`…, The context window to request, capped by what the model actually supports. Asks…, Ollama usually sends a dict; some builds send a JSON string., Models love to wrap JSON in ```json fences. Only unwraps when the fence is *all…, Promote a tool call the model wrote as *prose* into real `tool_use` blocks. Not… (+14 more)
...
Cohesion: 0.26
Nodes (22): _invoke(), _marker_path(), by_flavor_matrix, Root cause of "worked when it worked": the continuation the wrap-up forces is…, crew 1.0 F4 reachability fix. Before it, `context-watch.sh:81` / `context-…, A `delaySeconds` that is SET but not a usable number at all (unlike the…, _stop(), test_a_measured_drop_under_the_threshold_rearms_the_next_crossing() (+14 more)
...
Cohesion: 0.11
Nodes (19): _first_string_arg(), [(kind, value, line)] with kind 'id', 'str' or 'p' (one punctuation character;…, The first string argument of the call whose `(` is toks[j], else None., [(line, name, title)] for every skip site in a JS/TS source. `title` is the…, (paths, unresolved) for a playwright config's `storageState` values: literal…, skip_sites(), storage_state_values(), _tok() (+11 more)
...
### Community 279 - "sys"
Cohesion: 0.03
Nodes (75): config, dataclasses, fnmatch, hashlib, io, numpy, pathlib, Tests for gizmoduck.py's `doctor` command, specifically the NVD_API_KEY report… (+67 more)
...
Cohesion: 0.07
Nodes (47): git(), Throwaway git repositories and a fake reviewer CLI for the review-adapter…, test_a_rename_into_an_artifact_dir_still_judges_its_source(), test_audit_diffs_from_the_tickets_recorded_base(), test_activate_is_per_worktree(), The ticket-contract block of the review prompt: every piece is either present…, test_build_falls_back_to_the_files_mode_ticket(), test_build_includes_spec_sections_plan_and_receipts() (+39 more)
...
Cohesion: 0.16
Nodes (11): Second QA round, same day — each finding right about its target, wrong one line over, _iter_ndjson(), offered_tool_names(), _post_ollama(), _probe_num_ctx(), ProxyHandler, BaseHTTPRequestHandler, Always consume the request body, even on a path that ignores it.… (+3 more)
...
### Community 306 - "check_hard_block"
Cohesion: 0.11
...
Cohesion: 0.07
Nodes (62): Which ticket is active, functools, getpass, accepted(), activate(), _active_path(), active_ticket(), approval_path() (+54 more)

### Community 319 - "Verification harness"
Cohesion: 0.06
Nodes (23): Re-derive provenance, Code map — index, Coverage — and what is still unmapped, Files, How to read these files, Calls out to, Does, Entry points (+15 more)
...
Cohesion: 0.12
Nodes (26): Added, _ask(), _handoff_repo(), _lock_path(), _parallel_transcript(), needs_bash, parametrize, crew 1.0 T6 review fixes to the context hook, one repro per finding: the… (+18 more)
...
### Community 327 - "dedupe"
Cohesion: 0.17
Nodes (15): Task 3: Target-aware dedupe, 13.1 `dedupe()` collapses across targets — the spec's compatibility claim is wrong, 4. Tools, invocation, output → normalization, _categories_for(), category_of(), _cmd_report_combined(), cmd_tickets(), dedupe() (+7 more)

### Community 328 - "test_scope_guard_refresh_artifacts.py"
Cohesion: 0.22
...
### Community 329 - "test_gate_command.py"
Cohesion: 0.19
...
Cohesion: 0.17
Nodes (26): approval_hook, make_ticket(), prompt(), _hook(), _no_python_env(), fixture, parametrize, approval_hook.py and its .sh / .ps1 wrappers: the UserPromptSubmit hook that… (+18 more)
...
Cohesion: 0.07
Nodes (52): commit_file(), head_sha(), Short HEAD sha of a fixture repo., Stage and commit one path at the current time. The ordinary case.…, _git(), `knowledgeBehind` and `diagramsStale` had no fixpoint. Both compared `anchor ==…, The narrowing the docstring always described and the code never did. A map…, The third value must survive the change. `unresolvable` is exclusive of… (+44 more)
...
Cohesion: 0.19
Nodes (23): _check(), _codemap(), _git(), _on_main_with_a_map(), skipif, A repo on `main`, no scope record, with a code map for src/app.py anchored at…, test_a_chmod_000_codemap_dir_is_unknown(), test_an_untracked_file_git_would_quote_stales_the_graph() (+15 more)
...
Cohesion: 0.16
Nodes (20): completion_audit, env_for(), Throwaway repositories, tickets and hook runners for the crew 1.0 T3 suites…, Run one hook flavour. `stem` is `scope_guard` or `completion_audit`. Returns…, run_hook(), fixture, parametrize, T-0008 review round 1 (BLOCK): the completion audit accepts the refresh-… (+12 more)
...
### Community 404 - "worklog.py"
Cohesion: 0.17
Nodes (24): email_utils, EmailMessage, build_message(), Path, SMTP delivery for work log reports. Supports three transport shapes because…, recipients(), send(), load_session() (+16 more)
...
### Community 407 - "10. The daily loop"
Cohesion: 0.29
...
Cohesion: 0.12
Nodes (26): Added, Lifecycle commands, The artifact refresh check (T-0008, crew 1.0.36), Approval, Scope: approval and the completion audit, audit(), changed_paths(), _git_fields() (+18 more)
...
### Community 417 - "8. Verification, secrets, and browser tests"
Cohesion: 0.33
...
### Community 418 - "test_a_lock_call_error_that_is_not_unsupported_is_retried"
Cohesion: 0.33
...
### Community 421 - "test_a_cloned_repo_cannot_widen_the_machine_owners_choice"
Cohesion: 0.40
...
### Community 434 - "inspect_forms.py"
Cohesion: 0.17
...
### Community 456 - "commands/crew.md"
Cohesion: 0.33
Nodes (5): Step 0 — is crew even set up here, Step 2 — which roles are plausible, and which are not, Step 3 — the one supported route: a separate proxy session, Step 4 — end with one recommendation, What this command will not do
...
### Community 489 - "_repo"
Cohesion: 0.22
Nodes (21): _armed_or_silent(), by_flavor, A/link1 -> ../B/link2 -> inner/repo: each hop must resolve against the symlink…, alias -> B, A/link -> ../alias/inner/repo: `alias` is a symlink INSIDE the…, alias -> B/nested, A/link -> ../alias/../repo: the substituted target string…, A self-referential symlink in an onlyRepos entry must never resolve to a…, One key at a time, so neither can hide a widening of the other., `notify` types nothing, so `delaySeconds` buys it no wait -- printing the… (+13 more)
...
Cohesion: 0.13
Nodes (14): Commands, Configuration, Layout, Model spellings - verified, with controls, Registration, Relationship to crew, rule-of-two, The self-review (+6 more)
...
### Community 658 - "test_main_refuses_to_start_when_a_backup_is_present"
Cohesion: 0.17
...
### Community 700 - "`localgpu` — the GPU in this machine, as a sidecar"
Cohesion: 0.22
...
### Community 703 - "_make_stdout_safe"
Cohesion: 0.18
...
### Community 706 - "o365-admin/src/index.ts"
Cohesion: 0.36
Nodes (7): main(), CreatedServer, createServer(), delegatedScopes(), getClient(), buildServer(), fakeCredential

### Community 707 - "test_the_cli_announces_the_new_key_and_does_not_only_write_it_to_a_file"
Cohesion: 0.33
...
### Community 725 - "3. Requirements and platforms"
Cohesion: 0.33
...
### Community 752 - "24. The emergency lane"
Cohesion: 0.33
...
### Community 760 - "test_16_a_skip_after_a_pass_deletes_the_stale_fingerprint"
Cohesion: 0.40
...
### Community 761 - "resolve"
Cohesion: 0.07
Nodes (35): convert(), main(), page_colour(), Convert `src` with a DELIBERATELY chosen engine. Returns an exit code. This is…, The page colour a dark-page theme wrote into `path` -- the `doc-builder-page`…, build_from_spec(), configure(), main() (+27 more)
...
## Knowledge Gaps
- **2668 isolated node(s):** `ci-status.sh script`, `name`, `private`, `description`, `workspaces` (+2663 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 7425 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **153 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `[Unreleased]` connect `[Unreleased]` to `default_global_config`, `test_completion_audit.py`, `Brand`, `crew_incident.py`, `Fixed`, `rule_of_two.py`, `crew_endpoints.py`, `completion_audit.py`, `Fixed`, `reconcile`, `read_text`, `test_upgrade.py`, `collect`, `crew_instructions.py`, `crew_config.py`, `tg.py`, `crew_state.py`, `check-marketplace.py`, `test_crew_context_fixes.py`, `core/src/index.ts`, `search_code`, `main`, `test_check_reads_the_svg.py`, `verify_fingerprint.py`, `label`, `graphClient.ts`, `role_write_guard.py`, `Any`, `Changed`, `inbox.py`, `[2026-07-28]`?**
  _High betweenness centrality (0.021) - this node is a cross-community bridge._
- **Why does `codex()` connect `crew_instructions.py` to `Fixed`, `default_global_config`, `read_text`, `check_self_claims`, `rule-of-two`, ``obsidian-vault` — one or more Obsidian vaults as Claude Code memory`, `commands/crew.md`, `repo`, `localgpu`, `crew_state.py`, `Working with Codex`, `crew`, `rule_of_two.py`, `3. Requirements and platforms`, `Providers`, `_make_stdout_safe`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `Microsoft MCP servers` connect `Microsoft MCP servers` to `preflight.py`, `core/src/index.ts`, `README.md`, `.device`?**
