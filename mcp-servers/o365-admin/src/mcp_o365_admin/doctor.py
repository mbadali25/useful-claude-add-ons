"""``mcp-o365-admin-doctor`` - proves auth actually works, not just that the
...
def main() -> int:
    ...
    if writes_on:
        print(
            "  WARNING: writes are enabled on the highest-blast-radius server in this "
            "workspace (tenant-wide Mail/Sites/Teams/licensing). Confirm that is intended."
        )

    print("  probing sites?search=* ...")
    try:
        result = get_client().get("sites", params={"search": "*", "$top": 1})
        count = len(result.get("value", []))
        print(f"  OK - read call succeeded, {count} site(s) returned in this page.")
    except GraphError as exc:
        print(f"  READ FAILED: {exc}", file=sys.stderr)
        print(
            "  A valid token but a failed read usually means the app registration "
            "is missing Sites.Read.All (or broader) with admin consent granted.",
            file=sys.stderr,
        )
        return 1
