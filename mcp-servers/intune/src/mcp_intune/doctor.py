"""``mcp-intune-doctor`` - proves auth actually works, not just that the
command resolves on PATH. Decodes the acquired token's claims (no signature
verification - this is a display aid, not a trust decision) to show which
tenant and app the credentials belong to, then makes one real, harmless read
call so a scope/consent problem surfaces here instead of on the first real
tool call.
"""
...
def main() -> int:
    ...
    print("  probing deviceManagement/managedDevices?$top=1 ...")
    try:
        result = get_client().get("deviceManagement/managedDevices", params={"$top": 1})
        count = len(result.get("value", []))
        print(f"  OK - read call succeeded, {count} device(s) returned in this page.")
    except GraphError as exc:
        print(f"  READ FAILED: {exc}", file=sys.stderr)
        print(
            "  A valid token but a failed read usually means the app registration "
            "is missing DeviceManagementManagedDevices.Read.All (or broader) with "
