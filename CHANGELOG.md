...
- **`crew` 0.19.27: guardrails you can turn down per machine, a `/crew:gate`
...
- **`crew` 0.19.27: `/crew:config --explain` contradicted the run for every
...
- **`crew` 0.19.27: the schema-5 migration told the CLI nothing.** It added
...
- **`crew` 0.19.27: three `consider-using-f-string` warnings from 0.19.25's
