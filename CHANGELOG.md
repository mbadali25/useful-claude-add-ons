...
- **`crew` 1.0.40: auto-resume after `/clear`, reduced form (T-0006).** Bumped
  `1.0.37 -> 1.0.40`, the owner's land-order assignment of 2026-09-25
  (1.0.38 and 1.0.39 are not used here). New key
...
- **`crew` 1.0.29: the PATH stubs in `test_34d` and `test_1` are reached on
  ...
  - **`test_1` gets the same treatment** (review round 1): its CRLF
    `python3` stub now writes a counter that is asserted nonzero before any
    other assertion, and its sh flavour launches through the non-prepending
    bash on Windows. Before, renaming the stub so it could never be found
    still passed; now it fails at `assert 0 > 0`. The ps1 flavour is
    unchanged.
  - Off Windows `resolve_bash_no_prepend()` returns None and nothing calls
    it - both tests use `resolve_bash()`'s bash there, which prepends
    nothing. `resolve_bash()` stays the default for every other `sh` test.
  - **Enumeration: every sh test that puts a stub dir on PATH was checked;
    the ones found outside this ticket are deferred, not fixed.**
    `test_auto_clear_review_fixes.py:307` stubs `mktemp` with nothing proving
    the stub ran, so on Windows it can pass while exercising the real one;
    `test_auto_cycle.py:1021` stubs `bash`, which Git also ships (should fail
    loudly there, not pass); `test_auto_cycle.py:313` stubs `tmux`, which Git
    does not ship. Each is filed in `TODO.md` with its reason.
