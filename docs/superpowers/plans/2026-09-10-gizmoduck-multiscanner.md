...
| 13 | `sqlmap` | web (gated) | **True** | `sqlmap-session/` | `sqlmap -u <url> --batch --time-limit=<n> --output-dir=<dir>` | confirmed injection → `high`; stacked/UNION with DB access → `critical` | **No JSON exists.** Parse the **`log`** file for `Parameter:` / `Type:` / `Title:` / `Payload:` blocks — **not `session.sqlite`**, which is an opaque zlib+pickle HashDB cache (spec §13.9). A run with no persisted injection yields **zero findings**, not a low-severity one — and exit code 0 proves nothing. `run()` must refuse before building argv unless `opts["confirm"]` is set **and** the target carries a query string: a blind sweep is never acceptable |
