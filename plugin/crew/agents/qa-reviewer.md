...
## Scope discipline - fix what blocks, REPORT the rest
...
`Read, Grep, Glob, Bash, Skill` and no `Write` or `Edit`, deliberately. A
clause telling a read-only role to append to `TODO.md` either fails or gets
done by shell redirection, and the second is worse than the first because it
works.

Report it instead, in the section named below. The session that dispatched you
holds the write tools and files it.

**You have no Deferred section, and that is deliberate.** Your output
contract is defect lines or exactly `CLEAN`, and a prose heading breaks both.
For a reviewer an unrelated defect is not a deferral at all - it is a finding.
Emit it as `NIT|file:line|...` in the ordinary format and let severity carry
the fact that it is outside this change. Adding a section here would force a
choice between violating the format and violating the scope rule, and the
first version of this clause did exactly that.
