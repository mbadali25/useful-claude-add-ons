...
| `autoClearInert` | nobody — tell the user the one line to set | `context.autoClear.enabled` is true, the platform is Windows, and `context.autoClear.windowTitle` is unset. The native-Windows flavour has one method, SendKeys against a title-matched foreground window, and it refuses outright without a title rather than typing into whatever has focus — so auto-clear reads as ON and can never fire, and every refusal goes to `.crew/.autoclear.log` where nobody looks. Not a job for a role: it is one config key. **Detect it, do not guess it and do not ask blind.** Run `python3 ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/crew_state.py --detect-window-title`, which reads the real window titles from the OS and returns each with a `stable` flag. Show the candidate and, when `stable` is false, the reason it may stop matching — a Windows Terminal title follows the active tab, so a value correct now can be wrong on the next switch. Then offer the concrete line and let them choose; propose a substring that survives a tab change over the full title. **Setting it is still theirs, not yours:** which window gets typed into is a decision, and a detected title is evidence for that decision rather than a substitute for it. |
