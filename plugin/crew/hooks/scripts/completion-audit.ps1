...
# the audit caused is never blocked again even when python is broken.
...
if (-not $py) {
  if (Test-ScopeOptedIn) {
    [Console]::Error.WriteLine("COMPLETION AUDIT: no usable python - failing closed because scope.mode is block or auto.")
    exit 2
  }
  [Console]::Error.WriteLine("COMPLETION AUDIT: no usable python - not judged.")
  exit 0
}
...
if ($exitCode -ne 0 -and $exitCode -ne 2) {
  if (Test-ScopeOptedIn) {
    [Console]::Error.WriteLine("COMPLETION AUDIT: completion_audit.py did not run to a decision (exit $exitCode); failing closed because scope.mode is block or auto.")
    exit 2
  }
  [Console]::Error.WriteLine("COMPLETION AUDIT: completion_audit.py did not run to a decision (exit $exitCode); not judged.")
  exit 0
}
exit $exitCode
