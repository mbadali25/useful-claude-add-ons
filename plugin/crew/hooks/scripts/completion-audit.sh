...
# off, which from bash means the file is absent -- and blocks at most ONCE IN
# A ROW per session and project (a marker file, below), so a python that stays
# broken can never loop the Stop.
