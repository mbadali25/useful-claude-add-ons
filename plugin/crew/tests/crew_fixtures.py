...
def resolve_bash_no_prepend():
    ...
    executability probe `resolve_bash()` uses (`_usable`). On a non-
    Windows host there is no prepending launcher to route around, so this
    just returns whatever `resolve_bash()` already resolved.
    """
