...
### Auto-resume (on by default since 0.19.52)

`context.autoResume` opens the next `SessionStart` already holding the last
handoff and its next action. With `memory.inject: true` the context hook does
this inside its 3,000-character resume budget, with the next action leading so
a long note cannot push it out. It is implemented as `additionalContext`, not
