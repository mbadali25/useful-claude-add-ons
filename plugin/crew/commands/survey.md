...
1. Invoke the `crew:explorer` subagent. Give it the area, `.crew/codemap/INDEX.md`,
   anything you already know is painful, and ask for gaps with a `path:line`
   evidence line and a confidence for each.
2. For a broad survey, run explorer once per subsystem in parallel rather than once
   over everything — a single pass over a large repo produces shallow findings.
   Cap at four.
3. Read what each one returns; explorer writes no file.
...
- Anything explorer flagged as low confidence that you think is actually right

Do NOT create tickets. I decide what becomes work; `/crew:brainstorm` is a separate
