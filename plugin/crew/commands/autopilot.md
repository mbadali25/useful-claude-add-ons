...
never fix and rerun inside the phase (review.md step 3.2 is the human's call
here). Go back through `next`: it stops at FINDINGS or INCOMPLETE and puts a
refresh before any later round.
...
around it or edit a gate to get past it.

Note that every implement ends in an approval stop: its step 7 writes
`status: review` into spec.md, which the approval hashes whole, so `next`
stops at `approve` saying only the header changed (T-0026).
...
Always a person (policies are T-0010): `brainstorm` (no direction, or it is
unapproved), `plan-approval` (the human types `/crew:approve <ticket>`;
autopilot never runs it or writes an approval), `review-acceptance` (FINDINGS
are the owner's), `open-questions` (any item under `## Open questions` in
direction.md, spec.md or plan.md).
...
`unsettled-artifact`, `ticket-mismatch`, `max-phases`, `no-progress`.
