...
|---|---|
| **Source** | [`crew/`](crew) |
| **Version** | 0.19.52<!-- claim: plugin-version:crew --> |
...
| **Registers** | 54 agents, 26 commands, 18 skills<!-- claim: plugin-skills:crew -->, 18 hook entries (9 scripts × `.sh`/`.ps1`) across 5 events |
...
| `pm` | read/write + `Agent` | `opus` | — (outside the ladder) | The standing manager. Reads state, decides what the crew does next, and — **when `pm.authority` is `act` or above** — dispatches the roles that do it. Under the default `report-only` it recommends and stops; under `autonomous` it also settles its own open decisions instead of asking. Also does the heavy analysis that would cost more context in the main session than the answer is worth: correlating defect classes across `.crew/metrics.md`, auditing codemap anchors, assembling tier-change evidence |
...
| **Version** | 0.5.2<!-- claim: plugin-version:gizmoduck --> |
...
| **Version** | 0.1.18<!-- claim: plugin-version:localgpu --> |
...
| **Version** | 0.3.8<!-- claim: plugin-version:obsidian-vault --> |
...
| **Version** | 0.1.2<!-- claim: plugin-version:rule-of-two --> |
