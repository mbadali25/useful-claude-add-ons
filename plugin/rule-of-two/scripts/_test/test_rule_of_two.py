...
# Every entry is a deliberate exception with a reason AND is reachable by the
# regex above: an entry nothing can match is not a harmless spare, it
# pre-authorises a token nobody audited. Three were dropped for exactly that
# on the first pass - `_make_stdout_safe` and `model_id` (as `*_model_id`)
# both sit behind a leading `_`, where `\b[a-z]` cannot match, and crew's key
# is spelled `qa.order`, which the regex cannot produce.
NON_PAYLOAD_TOKENS = {
    "rule_of_two": "the script's filename",
    "render_method_note": "a function in rule_of_two.py",
    "author_families": "crew's config key, named in config.md to say this "
