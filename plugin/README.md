...
3. Register each hook **once, as bash**, and branch inside the script. There is no `shell` field in a `hooks.json` entry — Claude Code does not read one, so a hook registered with `shell: powershell` is silently inert, which reads as "the gate passed" rather than "the gate never ran". A hook that judges a *command* branches on `tool_name` (a `Bash` call is bash syntax even on Windows); a hook that judges no command is reached through `bash` and needs no `.ps1` at all. Any hook that can **block** also needs a committed regression suite with must-block and must-allow cases, sabotage-tested so you know it can go red — see `plugin/crew/hooks/scripts/_test/`.
