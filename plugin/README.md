...
3. Ship hook scripts in **both** flavours — a `.sh` for Linux/macOS and a `.ps1` registered with `shell: powershell` — and **wire both in `hooks.json`**. A `.ps1` that exists on disk but is never referenced is dead code, and a bash-only hook is silently inert on Windows, which reads as "the gate passed" rather than "the gate never ran".
