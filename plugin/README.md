...
| [`rule-of-two`](rule-of-two) | Review | Runs two adversarial reviewers from different model families — a Claude agent on Fable and a Codex agent on `gpt-6-astra` — over the same Claude Code agent, skill or plugin, against one shared rubric, with neither seeing the other's findings. Returns a viability verdict, strengths, weaknesses, cited defects and the edits worth making. Its headline reports coverage rather than assuming it: only one reviewer running, or two models that will not resolve to different families, is stated in the report rather than hidden behind the name. | Wanting a genuinely adversarial second opinion on a skill or plugin before shipping it; checking whether a new agent's description will actually trigger; deciding whether an artifact duplicates something that already exists. | 2 commands, 1 agent |
