...
```powershell
irm 'https://raw.githubusercontent.com/mbadali25/useful-claude-add-ons/1b19e5d80513cbd170a7271d59bd246c98e0ec03/scripts/install-prerequisites.ps1' | iex
```

**Linux**:

```bash
curl -fsSL 'https://raw.githubusercontent.com/mbadali25/useful-claude-add-ons/1b19e5d80513cbd170a7271d59bd246c98e0ec03/scripts/install-prerequisites.sh' | bash
```
...
| 25 Perplexity | `npx -y @perplexity-ai/mcp-server`, registered with `--env PERPLEXITY_API_KEY=<key>` so the CLI has the key when it launches the server. Unlike row 21 this **does** write the key into the MCP registration, so the server gets it whatever environment the `claude` process was launched from. The key comes from `--perplexity-api-key` / `-PerplexityApiKey` or `PERPLEXITY_API_KEY`, is never printed, and with neither set the row explains how to get one and skips. This is the server the [`web-research`](skills/web-research/) skill calls | `claude mcp add` |
